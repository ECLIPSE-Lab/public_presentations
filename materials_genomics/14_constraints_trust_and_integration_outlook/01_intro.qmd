...
**Pacing (rebalanced for the Week-13 fold-in).** Six minutes on §A (recap and unfinished business); twelve on §A2 (the folded-in Week-13 uncertainty / discovery-loop core); fifteen on §B (constraints, the materials-specific bit); six on §C (PINN reminder, light); fourteen on §D (distribution-shift trust); eighteen on §E (the autonomous-lab loop, the centrepiece); six on §F (FAIR / model cards / benchmarks); eight on §G (2026 outlook); five on §H (course wrap, exam framing). **This is full** — if you run long, compress §F (FAIR is on the slides) and trim §G to the headline.
...
**Said as a frame for the rest of the lecture.** "These three gaps are what today fills — after a twelve-minute §A2 that hands you the Week-13 tools you need. Fifty-six slides. Hold tight."
:::
...
**The five "must-know" statements (introduce now, repeat in §H slide 54).**
...
**Tell them where the exam questions live.** Each of the five statements is a likely exam stem. The "discovery-loop economics" calculation (§G slide 50) is also fair game.
:::
...
**Set expectations.** Six slides, one worked intuition each. If you took MFML, you saw the general uncertainty machinery in MFML W12; this is the materials-discovery instantiation.
:::
...
**Tie to MFML.** Students who took MFML W12 saw aleatoric / epistemic formally. Here the point is operational: $\sigma$ is the currency the discovery loop spends.
:::
...
## 20. PINN in One Slide (MFML W13 reminder)
...
**The soft-constraint nature of PINN.** $\lambda_r$ and $\lambda_b$ are exactly the soft-penalty $\lambda$ from §B slide 17. PINN training is *the* canonical example of soft-penalty constrained learning. The MFML W13 deck spends a whole section on $\lambda$-balancing because of this — and that section is **not in MG**.
...
**The experimentalist's heuristic.** "If your DFT screen says yes, your synthesis says yes 30–60% of the time." That is the order of magnitude. Plan capacity accordingly. Multi-fidelity active learning (§G slide 49) is partly designed to cope with this gap.
...
- Acquisition (EI / UCB / TS, slide 08) picks the *highest-$\alpha$* candidate.
...
- The audit trail is the **model card** (§F slide 44) — written once per loop run.
:::
:::
...
**Forward link to §F slide 46.** A reviewable materials-ML submission in 2026 includes the audit trail. Not the *whole* trail (50000 lines is unreadable), but a model card and dataset card that summarise it. Slide 46 will lay out the format.
...
## 43. Dataset Cards for Materials
...
## 44. Model Cards for Materials Surrogates
...
## 45. Benchmark Hygiene and the Materials Project Debates
...
## 46. What a Reviewable Materials-ML Submission Looks Like in 2026

::: {.columns}
::: {.column width="50%"}
**The minimum bundle**

1. Dataset card (slide 43).
2. Model card (slide 44).
3. Training script + environment lock (slide 42).
4. Random-split + LOCO + OOD numbers (slide 45).
...
::: {.notes}
**This slide is the §F summary in operational form.** Slides 42–45 explained the why; slide 46 is the *what to ship*. Six items, hard line. A submission that misses any of items 1–5 is not at the 2026 bar; a submission that misses 6–8 is at the bar but not a strong example of it.
...
## 47. Foundation MLIPs as the New Substrate
...
**Pointer to the next slide.** Slide 48 will lay out what *changes* about MG when foundation MLIPs are available — the bottleneck shift, the new economics. This slide just names the substrate.
:::

## 48. What Foundation MLIPs Change
...
- Multi-fidelity AL (slide 49) becomes the natural framing.
:::
:::
:::
...
**Multi-fidelity active learning is the natural framing.** Three fidelities now: foundation MLIP (cheapest, ~1 ms/structure, broadly accurate), DFT (expensive, ~hour/structure, more accurate), experiment (most expensive, days/measurement, ground truth). Multi-fidelity GP / BO routes queries to the cheapest sufficient fidelity. Slide 49 covers this.
...
## 49. Multi-Fidelity Active Learning
...
**Forward link.** Slide 51 will name the open problems — the gaps where the economics still does not work yet, the places where the loop is not yet self-paying. Slide 52 will close §G with the honest 2026 status.
:::
...
**Forward link to §H.** §H is the course-wrap. We have done the field-wrap; now we close the *course*. Slide 53 zooms back to MG's syllabus arc; slide 54 names the four big skills; slide 55 is the reading list; slide 56 is the exam. Five minutes. Then questions.
:::

# §H · Course Wrap {.section}

## 53. The MG Syllabus Arc in One Slide
...
::: {.notes}
**Why repeat the arc again at slide 53.** Slide 02 walked the arc looking forward (here is what we built). Slide 53 walks it looking back (here is what we used). The repetition is the pedagogical bookend: the same path traversed in two directions cements it.
...
## 55. Reading List for Going Further
...
## 56. Exam, Questions, and End of MG

::: {.columns}
::: {.column width="50%"}
**Exam scope**

- The four big skills (slide 54), each via one operational scenario.
...
**The closing line, said calmly.** "Thank you. This was Materials Genomics. The course tried to prepare you to do four things end-to-end. If it succeeded, you are ready to start a master's thesis or a PhD in this area. If it did not, the reading list (slide 55) is your second pass. Questions?"
