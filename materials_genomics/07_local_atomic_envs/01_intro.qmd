---
title: 'Materials Genomics<br>Unit 7: Local Atomic Environments & Universal MLIPs'
author:
...
- **Unit 6:** crystals as *graphs* — atoms are nodes, neighbours are edges, GNNs learn the aggregation.
- **MFML W5 / W9:** latent-space geometry, learned representations.
- **ML-PC W2:** PCA / SVD as the linear unsupervised baseline.
:::
::: {.column width="50%"}
**Today — Unit 7 in one line**
...
::: {.notes}
**Position the unit.** Unit 7 is *not* a derivation unit. It is the unit where students learn to *choose* a representation under constraints. The mathematical machinery (Gaussian basis functions, spherical harmonics, kernel similarity) was largely covered in MFML; we use the results.

**Anchor to Unit 6.** A graph neural network is, conceptually, a *learned* atom-centred descriptor: it builds a feature on each node by aggregating from neighbours, then aggregates again. ACSF and SOAP do the same thing but with the aggregation *fixed by hand*. Today's lecture lets you read the GNN literature with informed eyes — you can ask "what aggregation is this layer reproducing, and what aggregation is it adding?"
...
4. **Learned graph** — GNNs (Unit 6). The aggregation itself is learned.
:::
::: {.column width="50%"}
```{mermaid}
graph TD
...
**Cross-reference.** Tier 4 was Unit 6's whole topic. Tier 3 is today's main content. Tier 2 is §A's structural-baseline material. Tier 1 is §A's compositional-baseline material. Lecturer note: *do not* let students conflate "structure-aware" with "tier 3+". Tier 2 is also structure-aware; it just pools differently.
:::
...
**Cross-reference to Unit 4.** MD simulations naturally produce RDFs at finite temperature; that is one way the simulation pipeline of Unit 4 feeds the descriptor pipeline of Unit 7.
:::
...
**Cross-reference to Unit 6.** Tier 2 is the closest hand-built analogue to a one-layer GNN: aggregate from neighbours, then pool the aggregations. The difference is that the GNN learns the aggregation function, and tier 2 fixes it as "compute these moments".
:::
...
**Cross-reference to Unit 6.** Graph neural networks define $\mathcal{N}(i)$ as the graph's edge set; the *graph* is the engineered prior, and the GNN learns aggregations on top. Tier 3 is the same prior, expressed at the descriptor level rather than at the message-passing level.
...
**Cross-reference to Unit 6.** GNNs solve the same three problems by message passing: edges are defined by relative positions, sums over edges are permutation-invariant, and local features are computed in a translation- and rotation-aware way. Different machinery, same principles.
:::
...
**Cross-reference.** Voronoi neighbourhoods are the connection between the *crystal-as-graph* picture of Unit 6 (where edges are Voronoi face contacts) and the descriptor picture of Unit 7. The graph-edge construction in MEGNet, M3GNet, and CHGNet is essentially Voronoi-derived.
:::
...
**A common confusion.** "Pooling" in this lecture is per-atom-to-per-material aggregation. In Unit 6 / GNN literature, "pooling" can also mean per-edge-to-per-node or per-graph aggregation. Same word, different scopes. We are doing the *graph-to-vector* end of the chain.
...
**Cross-reference.** This is the same "global mean masks local minority" failure that Unit 6 GNN literature responds to with attention pooling and node-classification heads. Same disease, different cure.
:::
...
**Cross-reference.** Unit 6's MEGNet and CHGNet use *both* mean and sum pooling in different output heads (intensive vs extensive). The architecture-level distinction we are making here is exactly that.
...
- &larr; Previous: [Unit 06 &mdash; Graph-Based Crystal Representations](../06_graph_based_rep/01_intro.html)
- &rarr; Next: [Unit 08 &mdash; Regression and Generalization in Materials Data](../08_regression_and_generalization_in_materials_data/01_intro.html)
