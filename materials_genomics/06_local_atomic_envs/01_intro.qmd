---
title: 'Materials Genomics<br>Unit 6: Local Atomic Environments'
author:
...
::: {.notes}
**Open with the question, not the title.** "Last week we drew crystals as graphs. The week before, we treated them as quantum-mechanical objects. Today we ask the most boring possible question — *how do you turn a crystal into a vector* — and discover that the answer is the most consequential design decision in any materials ML pipeline."
...
- **Unit 5:** crystals as *graphs* — atoms are nodes, neighbours are edges, GNNs learn the aggregation.
- **MFML W5 / W9:** latent-space geometry, learned representations.
...
**Anchor to Unit 5.** A graph neural network is, conceptually, a *learned* atom-centred descriptor: it builds a feature on each node by aggregating from neighbours, then aggregates again. ACSF and SOAP do the same thing but with the aggregation *fixed by hand*. Today's lecture lets you read the GNN literature with informed eyes — you can ask "what aggregation is this layer reproducing, and what aggregation is it adding?"
...
**Forward link to MFML W9.** Outcome 1's notion of a "baseline" connects to MFML's bias-variance discussion. Outcome 7 connects to ML-PC's leakage and out-of-distribution material. Both will be re-emphasised in Unit 7 next week.
:::
...
4. **Learned graph** — GNNs (Unit 5). The aggregation itself is learned.
:::
::: {.column width="50%"}
```{mermaid}
graph TD
    L1[1. Composition only<br/>Magpie / matminer comp.]
    L2[2. Globally-pooled<br/>RDF, aggr. coordination]
    L3[3. Atom-centred local<br/>ACSF, SOAP, bond stats]
    L4[4. Learned graph<br/>GNNs, MEGNet, M3GNet]
    L1 --> L2 --> L3 --> L4
    L1 -.->|"polymorph-blind"| F1[fails on TiO2 rutile vs anatase]
    L2 -.->|"motif-blind"| F2[fails on tet vs oct sites]
    L3 -.->|"long-range-blind"| F3[fails on band topology]
```
:::
:::
...
**Cross-reference.** Tier 4 was Unit 5's whole topic. Tier 3 is today's main content. Tier 2 is §A's structural-baseline material. Tier 1 is §A's compositional-baseline material. Lecturer note: *do not* let students conflate "structure-aware" with "tier 3+". Tier 2 is also structure-aware; it just pools differently.
:::

## 05. Tier 1 — Magpie elemental statistics
...
## 06. Tier 1 — The matminer feature library
...
**Forward to Unit 7.** Unit 7 will reframe the baseline question as part of *split design*: "does the structure-aware model still beat composition under chemistry-disjoint splits?" Often, the gap shrinks further. Today: report the baseline. Next week: stress-test it.
:::
...
## 10. Tier 2 — Structure-aggregated coordination
...
**Cross-reference to Unit 5.** Tier 2 is the closest hand-built analogue to a one-layer GNN: aggregate from neighbours, then pool the aggregations. The difference is that the GNN learns the aggregation function, and tier 2 fixes it as "compute these moments".
:::
...
```{mermaid}
graph TD
    Q1{Polymorph- or defect-sensitive target?}
    Q1 -->|"No"| T1[Tier 1: Magpie / matminer composition. Done.]
    Q1 -->|"Yes"| Q2{Property controlled by per-site motif?}
    Q2 -->|"No, isotropic-structural"| T2[Tier 2: RDF or aggregated coordination.]
    Q2 -->|"Yes, motif-controlled"| Q3{Long-range physics in the target?}
    Q3 -->|"No"| T3[Tier 3: ACSF / SOAP, then pool.]
    Q3 -->|"Yes"| T4[Tier 4: GNN, or tier 3 + global features.]
```
...
**Cross-reference to Unit 5.** Graph neural networks define $\mathcal{N}(i)$ as the graph's edge set; the *graph* is the engineered prior, and the GNN learns aggregations on top. Tier 3 is the same prior, expressed at the descriptor level rather than at the message-passing level.
...
**Cross-reference to Unit 5.** GNNs solve the same three problems by message passing: edges are defined by relative positions, sums over edges are permutation-invariant, and local features are computed in a translation- and rotation-aware way. Different machinery, same principles.
:::
...
**A worked microscopic example.** Pt(111) terrace site: coordination 9. Pt(111) step-edge site: coordination 7. Pt(111) kink site: coordination 6. These are the catalytically interesting sites, and they *are* distinguishable by coordination. But once you go to fcc Pt corner sites with very different *angular* distribution at coordination 5–6, count alone fails; you need angular features to keep them apart.

**The Jahn–Teller example.** Octahedral Mn$^{3+}$ in $\mathrm{LaMnO}_3$ shows a coordination-6 environment with two long Mn–O bonds and four short ones. Average bond length is unaffected; bond-length variance distinguishes Jahn–Teller-distorted from regular octahedra. This is *exactly* the kind of distinction that bond-length statistics (slide 22) capture and coordination misses.
...
**A real dataset to reach for.** The Materials Project's spinel subset (~200 ABO$_2$ structures) is a clean test bed. Run matminer's `CrystalNNFingerprint` (slide 10) and `BondFractions` (slide 6) on it; you will see the tetrahedral / octahedral families separate cleanly in the first two PCA components. This is a five-minute exercise students can do at the laptop.
...
**Cross-reference.** Voronoi neighbourhoods are the connection between the *crystal-as-graph* picture of Unit 5 (where edges are Voronoi face contacts) and the descriptor picture of Unit 6. The graph-edge construction in MEGNet, M3GNet, and CHGNet is essentially Voronoi-derived.
:::
...
## 28. ACSF — atom-centred symmetry functions
...
**Cross-reference.** Slide 35 (ACSF vs SOAP) is the explicit comparison; slide 36 captures the lecture's recommendation.
...
## 31. SOAP — smooth overlap of atomic positions
...
```{mermaid}
graph LR
    A[Local environment<br/>positions + species] --> B[Smooth density<br/>per species]
    B --> C[Spherical-harmonic<br/>+ radial expansion]
    C --> D[Coefficients c_nlm<br/>per species]
    D --> E[Power spectrum<br/>p_nn'l per species pair]
    E --> F[Cosine kernel<br/>per environment pair]
    F --> G[GP / KRR / NN<br/>regression]
```
...
**Forward link.** §E now turns to the question that every tier-3 descriptor must answer: how do we go from atom-level $\phi_i$ to material-level $\Phi$?
:::
...
**A common confusion.** "Pooling" in this lecture is per-atom-to-per-material aggregation. In Unit 5 / GNN literature, "pooling" can also mean per-edge-to-per-node or per-graph aggregation. Same word, different scopes. We are doing the *graph-to-vector* end of the chain.
...
**Cross-reference.** This is the same "global mean masks local minority" failure that Unit 5 GNN literature responds to with attention pooling and node-classification heads. Same disease, different cure.
:::
...
**Cross-reference.** Unit 5's MEGNet and CHGNet use *both* mean and sum pooling in different output heads (intensive vs extensive). The architecture-level distinction we are making here is exactly that.
...
**The split-design implication (preview of Unit 7).** Polymorph aliasing means random train/test splits are *too easy*: a polymorph in the test set is nearly identical to one in the training set. Polymorph-disjoint splits expose the aliasing; *that* is the test of whether your model captures polymorph physics. Unit 7 expands this into the leave-one-cluster-out methodology.
...
- *Be honest about uncertainty:* on long-range targets, tier-3 confidence intervals should be wider than the model claims (Unit 12).
:::
:::
...
**Cross-reference.** Unit 12 (uncertainty-aware discovery) revisits this as a calibration question: tier-3 models on long-range targets *should* have high posterior uncertainty in the framework regions where they alias; if they don't, the uncertainty is mis-calibrated.
:::
...
**Forward link.** §G now wraps up with the descriptor ladder summary and the bridge to Unit 7.
:::
...
## 51. Forward pointers — what Unit 7 will do with this

::: {.columns}
::: {.column width="50%"}
**Unit 7 — Regression and generalisation**
...
**A pedagogical anchor for next week.** Unit 7 will revisit *every* failure mode from §F today, but reframed as a *split-design* question: random splits hide aliasing; chemistry-disjoint splits expose it; polymorph-disjoint splits expose polymorph aliasing; structure-disjoint splits expose framework aliasing. The descriptor *and* the split together determine what generalisation means.
...
**Next week (Unit 7):** baselines, split design, learning curves, leakage, OOD — the trust audit on the regression you got today.
:::
:::
...
**Final check.** Verify before next week: every student has a working `dscribe` install, can load a Materials Project structure, and can run `SiteStatsFingerprint`. Without these, Unit 7's split-design exercise stalls.
:::
...
- &larr; Previous: [Unit 05 &mdash; Graph-Based Crystal Representations](../05_graph_based_rep/01_intro.html)
- &rarr; Next: [Unit 07 &mdash; Regression and Generalization in Materials Data](../07_regression_and_generalization_in_materials_data/01_intro.html)
