...
**Two documented cases that make this point — cite these, not an anecdote.** (1) *Matbench* (Dunn et al., npj Comput. Mater. 6:138, 2020) uses a Random Forest on Magpie/Sine-Coulomb descriptors as the reference baseline across 13 tasks. The result that survived peer review: the descriptor baseline has *lower* error than the CGCNN and MEGNet graph networks on every task below ~10⁴ samples; the graph nets only overtake past that scale. The durable framing is therefore "the composition baseline closes most of the gap; the deep model is a large-data refinement", which is exactly the lesson this slide teaches — and it is reproducible because the benchmark and splits are public. (2) *Bartel et al.* (npj Comput. Mater. 6:97, 2020): seven ML models predict formation energy with excellent MAE, yet all composition-only models predict convex-hull stability — the property anyone actually cares about — poorly. "Good MAE" was not the durable claim; "does it solve the downstream task?" was. Both papers institutionalise the discipline better than any single reviewer story: report the simple baseline, and state your result relative to it.
