...
- **MG U7:** crystals as graphs under PBC [@sandfeld_materials_data_science].
...
3. Implement, conceptually, a **CGCNN** message-passing step on a crystal graph and connect it to MG U7.
...
:::
::: {.column width="45%"}
![](images/diagram_0.png){height="650px" fig-align="center"}
:::
:::
...
::: {.notes}
Intuition: thermometer vs compass
Use a concrete analogy before the math.

Invariance (thermometer)
Rotate the crystal. Total energy does not change.
If you predict $E$, the network should give the same number regardless of orientation.

Equivariance (compass needle)
Rotate the crystal. Each force vector rotates by the same rotation.
If atom $i$ felt a force pointing east before rotation, after a 90° rotation it should point north — not stay "east" in lab coordinates.

So:

Scalar property → build an invariant model (distances, angles, scalars only).
Vector/tensor property → build an equivariant model (internal features that know about direction).

What are you predicting?
│
├─ Scalar only (E, band gap, ...)
│   └─ Invariant GNN is fine
│       Simple, fast, works well at 10⁵–10⁷ structures
│
└─ Forces / stress / dipole matter
    │
    ├─ Lots of data (10⁶–10⁷+ structures, foundation MLIP)
    │   └─ Invariant + autograd often enough (M3GNet, OMat24-style)
    │
    └─ Small/medium data (10³–10⁵ DFT structures, catalysts, alloys)
        └─ Go equivariant (NequIP / MACE / Allegro)
            ~10×–100× better data efficiency for forces
...
::: {.fragment}
> E(3) is not the whole story for crystals.
A good atomic ML model respects E(3)/SE(3) plus permutation plus periodicity. Papers say "E(3)-equivariant" because rotation/translation are the hard part architecturally; permutation and PBC are handled separately (slides 11+).
:::
...
::: {.fragment}
![](images/schnet_paper/fig2_architecture.png){width="32%" fig-align="center"}

*SchNet architecture [@schutt2017schnet]: atom embedding $\to$ interaction blocks (cfconv) $\to$ atom-wise MLP $\to$ sum readout $\hat{E}$.*
:::
...
- **Left:** discrete filter on a grid $\to$ jagged, discontinuous $\hat{E}(R)$.
:::
::: {.column width="50%"}
![](images/schnet_paper/fig1_discrete_vs_continuous.png){width="100%" fig-align="center"}

*Figure from [@schutt2017schnet]: discrete vs continuous filter; continuous $\to$ smooth energy surface.*
...
::: {.fragment}
![](images/schnet_paper/fig3_learned_filters.png){width="88%" fig-align="center"}

*Learned radial filters $W(r)$ in three SchNet interaction blocks [@schutt2017schnet] — rotationally symmetric by construction.*
:::
:::
:::
...
::: {.fragment}
> QM9 is the MNIST-like benchmark of early molecular machine learning: small, standardized, widely used, and focused on predicting quantum-mechanical properties from molecular structure.
:::
...
**Pre-empt.** "Why not just stack more CGCNN layers to capture longer range?" Answer: over-smoothing. Past $T \approx 6$, atom features become indistinguishable across the cell, and the network's effective expressivity *decreases*. This is not a CGCNN-specific bug; it is a general GNN phenomenon. MG U7 slide 17 covered it.
:::
...
:::
::: {.column width="45%"}
![](images/diagram_1.png){width="100%" fig-align="center"}
:::
:::
...
:::
::: {.column width="45%"}
![](images/diagram_2.png){width="100%" fig-align="center"}
:::
:::
...
**Cross-reference to the Unit 14 finale.** Uncertainty quantification on foundation-model predictions is a major open problem, and the Unit 14 finale covers it (the Week-13 lecture was cancelled; its uncertainty core is folded in there). Today: name the gap; do not pretend it is solved.
...
1. **Build a CGCNN baseline** on a curated Materials Project subset (formation energy or band gap). Use the MG U7 graph constructor; reuse MG U8's grouped-split protocol.
