---
title: 'Materials Genomics<br>Unit 9: Neural Networks for Materials Properties'
author:
...
- Not a re-derivation of crystal graphs — that is **MG U6**, recapped in one slide.
- Not a re-derivation of local environments — that is **MG U7**, recapped in one slide.
:::
:::
...
**Pre-empt the obvious objection.** "Why not just train an MLP on a Magpie vector?" Answer: you can, and you should as a baseline (MG U7, U8). But it cannot distinguish two polymorphs with the same composition — and most interesting properties depend on geometry. Today's architectures fix exactly that.
...
- **MG U6:** crystals as graphs under PBC [@sandfeld_materials_data_science].
- **MG U7:** local atomic environments, SOAP, ACSF.
...
**Anchor to MG U6.** "Last unit you built a crystal graph. Today every model we discuss eats exactly that graph and produces a property. CGCNN, MEGNet, ALIGNN are direct instantiations of last week's message-passing schematic."
...
3. Implement, conceptually, a **CGCNN** message-passing step on a crystal graph and connect it to MG U6.
...
8. Articulate the bridge to Unit 10: foundation-model embeddings as the central object of representation learning.
:::
:::
:::
...
## 05. Crystals as graphs (recap from MG U6)
...
::: {.fragment}
> **MG U6 contract still holds today.** PBC are enforced at graph-construction time; cutoff and RBF parameters are documented; reproducibility starts with a deterministic graph.
:::

::: {.notes}
**Recap, not re-derivation.** The MG U6 deck spent twelve slides on graph construction, PBC handling, cutoff vs k-NN, RBF expansion of distances, and reproducibility. Today we use that graph as the *input data type*. If a student is shaky on PBC graph construction, the MG U6 slides are the recovery path.
...
## 06. Local atomic environments (recap from MG U7)
...
- MG U7 introduced **fixed** local descriptors: SOAP, ACSF, Voronoi statistics — invariant by construction, hand-engineered.
...
::: {.notes}
**The single conceptual leap from MG U7 to MG U9.** MG U7 said: pick a fixed function $\phi(\text{local env}) \to \mathbb{R}^d$, pool, then regress. MG U9 says: let $\phi$ be a *learnable* neural network, train it jointly with the predictor, and the network will discover the right local descriptor for the target.
...
**One-sentence handoff.** "MG U7 gave us hand-crafted local descriptors; MG U9 gives us learned ones. The rest is plumbing."
:::
...
- This is a strong, common baseline (MG U7 / U8).
:::
::: {.column width="50%"}
**The failure mode**
...
**Why we still teach Magpie.** Composition is cheap and surprisingly strong as a *baseline* for many properties — see MG U7's Magpie discussion. The point of today is not to dismiss Magpie. The point is to recognise its ceiling. When that ceiling matters, we move to structure-aware architectures.

**Pre-empt.** "Can I just add structural features to the Magpie vector?" Answer: yes — that is what SOAP, ACSF, and pooled local environments do (MG U7). They climb the ceiling but never quite remove it. The natural endpoint is to let the network *itself* read the structure: that is SchNet, CGCNN, etc.
...
**Receptive-field intuition.** $T$ interaction layers $\to$ atom $i$ sees up to $T$ hops away. Typical $T = 3$–$6$. Beyond that, over-smoothing dominates (MG U6 already covered this).
:::
...
**Cross-reference to MG U6.** This was already covered in U6 slide 27 (cutoff artefacts). We re-mention it here because in the MLIP context (§E, §F), continuity of forces is non-negotiable — it is what makes the model usable in actual MD.
:::
...
**Why we are spending an entire next section on CGCNN.** Because CGCNN is the workhorse architecture for materials property prediction in 2018–2022 and remains a strong baseline today. Every MG U8 benchmark plot has a CGCNN bar in it.
...
**MG U6 cross-reference.** The graph construction we are reusing here is exactly the one we built in MG U6: PBC neighbour search, cutoff radius, distance encoding. We do not redo it.
:::
...
- Optionally enriched with chemistry priors (group, period, electronegativity, atomic radius) — see MG U6 slide 22.
...
## 23. Connection back to MG U6

::: {.columns}
::: {.column width="50%"}
**MG U6 sketched the schematic**
...
::: {.fragment}
> **The pedagogical reason for ordering MG U6 before MG U9.** U6 builds the *interface*; U9 fills in the *implementation*. Once a student has the U6 schematic, every architecture in §D–§F is a different way of filling slots in that schematic.
:::

::: {.notes}
**Repeat this connection out loud.** The MG U6 message-passing schematic is the *pattern*; CGCNN/MEGNet/ALIGNN/M3GNet/NequIP are *patterns instantiated*. Same five slots — graph, node features, edge features, message function, readout — different choices in each slot.
...
**Why this matters for the exercise.** When the students implement a CGCNN baseline, they will need to fill exactly these five slots. If MG U6's schematic is in their head, the implementation is a matter of plugging in the slots — not learning a new framework.
:::
...
**Pre-empt.** "Why not just stack more CGCNN layers to capture longer range?" Answer: over-smoothing. Past $T \approx 6$, atom features become indistinguishable across the cell, and the network's effective expressivity *decreases*. This is not a CGCNN-specific bug; it is a general GNN phenomenon. MG U6 slide 17 covered it.
:::
...
- Hand-crafted descriptors (MG U7) competitive.
...
- Similarity search and clustering — Unit 11 territory.
- Transfer learning and fine-tuning — already discussed in §G.3.
- Generative design (sample new $\mathbf{z}$, decode to a structure) — Unit 12 territory.
- Uncertainty quantification on $\mathbf{z}$-space neighbours — Unit 13 territory.
- Active learning loops driven by $\mathbf{z}$ — Unit 14.
...
- &larr; Previous: [Unit 08 &mdash; Regression and Generalization in Materials Data](../08_regression_and_generalization_in_materials_data/01_intro.html)
- &rarr; Next: [Unit 10 &mdash; Representation Learning and Feature Discovery](../10_representation_learning_and_feature_discovery/01_intro.html)
...
1. **Build a CGCNN baseline** on a curated Materials Project subset (formation energy or band gap). Use the MG U6 graph constructor; reuse MG U8's grouped-split protocol.
