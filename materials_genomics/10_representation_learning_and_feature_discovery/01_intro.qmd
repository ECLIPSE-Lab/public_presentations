...
- Generalises directly to the **active-discovery loop** (Week-13 core, delivered in Unit 14): "find me a candidate similar to this one".
:::
:::
...
**The methodology is what students should learn.** Active learning (foreshadow the discovery loop in Unit 14) plus a pretrained encoder is the right loop for materials discovery. GNoME was an early high-profile example. The exact numerical claims will be relitigated; the loop architecture is durable.

**Why this is in §E and not in §F.** Because GNoME is a pretrained encoder, and the encoder is what we are surveying here. The discovery-loop application lands in Unit 14 (the Week-13 lecture was cancelled and its core folded in there).
:::
