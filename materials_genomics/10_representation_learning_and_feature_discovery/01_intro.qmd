---
title: 'Materials Genomics<br>Unit 10: Representation Learning and Feature Discovery'
author:
...
- **MG U7:** Magpie / matminer / SOAP / ACSF — the engineered-descriptor baseline we have to beat.
:::
::: {.column width="50%"}
...
**Anchor to MG U7.** The Magpie / matminer / SOAP / ACSF descriptors from U7 are not deprecated. They reappear in §G as the baseline that any learned embedding must beat. *A learned embedding without a baseline comparison is unfalsifiable.*
...
- Magpie + matminer + tree model (MG U7 baseline).
- SOAP + GP (MG U7 baseline).
...
::: {.notes}
**Why MatBERT is in this lecture at all.** Because text is genuinely a materials modality and the U13 active-discovery loop will eventually want to read papers, not just structures. A text encoder is the right tool for that lane.
...
**The methodology is what students should learn.** Active learning (foreshadow U13) plus a pretrained encoder is the right loop for materials discovery. GNoME was an early high-profile example. The exact numerical claims will be relitigated; the loop architecture is durable.

**Why this is in §E and not in §F.** Because GNoME is a pretrained encoder, and the encoder is what we are surveying here. The discovery-loop application is U13.
:::
...
**Forward link.** This is the seed for U13's active-discovery loop. The retrieval check we run as a *diagnostic* in U10 becomes the *operational primitive* in U13.
:::
...
**Closing line.** "The embedding you trust today is the one we will feed into the generative models next week. See you in U12 — generative models and inverse design."
:::
...
- &larr; Previous: [Unit 09 &mdash; Neural Networks for Materials Properties](../09_neural_networks_for_materials_properties/01_intro.html)
- &rarr; Next: [Unit 12 &mdash; Generative Models & Inverse Design](../12_generative_models_and_inverse_design/01_intro.html)
