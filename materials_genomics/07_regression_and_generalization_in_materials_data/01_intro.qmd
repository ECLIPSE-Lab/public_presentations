---
title: 'Materials Genomics<br>Unit 7: Regression and Generalization in Materials Data'
author:
...
::: {.columns}
::: {.column width="50%"}
**Recap — Units 1–6 (MG) and the parallel tracks**
...
- *MFML W7 (parallel track):* bias-variance, regularization, $k$-fold CV, validation/test discipline — the generic ML pipeline.
- *ML-PC W7 (parallel track):* process windows, robustness, hyperparameter optimization on AM/process data.
:::
::: {.column width="50%"}
**Today — MG Unit 7**
...
**The triad-coordination message — say it loudly.** SS26 has three parallel ML tracks running this week. MFML W7 covered the generic regression pipeline: bias-variance, ridge/lasso, $k$-fold CV, validation/test discipline. ML-PC W7 covered process windows, robustness to measurement noise, hyperparameter optimization, and sensitivity analysis on additive-manufacturing data. **MG U7 deliberately does not re-cover any of that.** Re-teaching MFML W7 here would be the wrong use of 90 minutes — and would suggest to students that the materials-specific failure modes are a secondary concern. They are the primary concern.
...
**Forward links.** Unit 8 (next week) introduces neural surrogates — SchNet, CGCNN, MEGNet — and the validation discipline from today carries forward unchanged. Unit 12 (uncertainty-aware discovery) puts calibrated error bars on regression predictions and uses today's split-design taxonomy as the foundation.
...
7. **Audit** a published materials-regression paper against the MG U7 trustworthy-reporting checklist.
:::
:::
:::
...
- Calibrated uncertainty (conformal prediction, GP error bars) — MG Unit 12.
- Neural-network architectures for materials — MG Unit 8 (next week).
...
**Notation discipline.** I use $\mathbf{x}_i \in \mathbb{R}^d$ for the material-level descriptor (the output of Unit 6's pooling step), $y_i \in \mathbb{R}$ for the scalar target (formation energy in eV/atom, band gap in eV, bulk modulus in GPa, etc.), and $\boldsymbol{\theta}$ for whatever model parameters we are fitting. Same notation as MFML W7.
...
**The fix at the data level.** Active learning, targeted high-throughput campaigns to fill coverage gaps, transfer learning from broader simulations (e.g., Open Catalyst Project, Alexandria, MatBench-Discovery test sets), and explicit reweighting during training. These are MG Unit 12 content.
...
```{mermaid}
graph LR
    F[Feature space] --> H[Train hull]
    H --> I[Inside: interpolation]
    H --> O[Outside: OOD/extrapolation]
    I --> R1[Random splits live here]
    O --> R2[Where discovery actually happens]
```
:::
::: {.column width="50%"}
...
**Cross-database training as a partial fix.** Models trained on multiple databases simultaneously, with database identity as an input feature, can learn the inter-database biases and subtract them out. This is starting to happen (e.g., Alexandria + MP joint training); it is an active research direction. Mention by name; it's MG Unit 12-ish content.
...
**Calibration techniques.** When experimental data is available for a subset, simple linear calibration ($y_{\text{exp}} = a \, y_{\text{DFT}} + b$ per chemistry family) can shift predictions toward the experimental scale without retraining. More sophisticated: domain adaptation with a small experimental fine-tuning set. MG Unit 12 covers this with calibrated uncertainty.

**Forward link.** Unit 8 introduces neural surrogates trained on DFT. The DFT-vs-experiment domain shift survives Unit 8 unchanged; better architecture does not fix the target-definition problem.
:::
...
**The data-collection fix.** Active-learning campaigns that target high-energy-above-hull phases can rebalance the training distribution. This is happening (e.g., GNoME, MatterSim). It's MG Unit 12 content.
...
**The research frontier.** Active learning on disorder-relevant configurations, machine-learned interatomic potentials trained on disordered MD trajectories (MACE, Allegro, MatterSim), explicit configurational sampling. MG Unit 12 / Unit 13 territory.
...
**For active learning.** Each iteration of an active-learning loop adds new materials to the labelled pool. The natural train/test boundary is "before this iteration / after this iteration". Plotting model performance vs iteration is a direct measure of the active-learning policy's effectiveness — this is MG Unit 12 content.
...
**A useful citation.** "I followed the split-design recommendations of MG U7 / MatBench / [Sutton et al., npj CompMat 2020 on identifying domains of applicability]." Build a citation chain; the practice spreads.
:::
...
**Connect to MG Unit 12.** Calibrated uncertainty (GP error bars, conformal prediction) is the principled way to estimate per-prediction noise — including target noise plus model uncertainty. Mention now; it's Unit 12 content.
:::
...
**Connect to MG Unit 12.** Active-learning acquisition functions (UCB, EI, Thompson sampling) use uncertainty-weighted ranking. The ranking-vs-calibration distinction matters there; well-calibrated uncertainty is the requirement, not just well-calibrated point predictions.
...
## 47. The MG U7 Trustworthy-Reporting Checklist
...
## 50. Bridge to Unit 8 — Neural Networks for Materials Properties
...
::: {.fragment}
> *Better architecture does not fix bad benchmarking. The MG U7 discipline carries into U8 and every subsequent unit of MG.*
:::

::: {.notes}
**The bridge to U8 is conceptual, not architectural.** U8 will introduce new modeling tools; the *evaluation discipline* from U7 carries forward unchanged. The cohort should leave knowing that "GNN" is not a magic word that exempts a paper from the §F checklist.

**A specific anti-pattern U8 will encounter.** The "we built a fancy GNN that achieves 28 meV/atom MAE on MP formation energies, beating prior work by 2 meV/atom" paper format. By U7's standards: where are the baselines? What's the chemistry-aware MAE? What's the per-prototype breakdown? Did the structure-randomization ablation pass? A 2 meV/atom improvement at the global MAE level often disappears under any of these probes. The U8 students will read papers like this critically because they passed U7.

**Forward links into the rest of the MG unit chain.**
- U8: neural surrogates (next week).
- U9: representation learning, pretraining, multi-task models.
- U10: physics-informed and equivariant models.
- U11: generative models for inverse design.
- U12: uncertainty-aware discovery — calibrated GP and Bayesian NN error bars on the predictions we made today.
- U13: active learning loops — choosing the next experiment given a regression model and an uncertainty estimate.
...
- &larr; Previous: [Unit 06 &mdash; Local Atomic Environments](../06_local_atomic_envs/01_intro.html)
- &rarr; Next: [Unit 08 &mdash; Neural Networks for Materials Properties](../08_neural_networks_for_materials_properties/01_intro.html)
