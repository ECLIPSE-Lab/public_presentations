...
- Materials Genomics operationalizes discovery as a data-driven design process. {.fragment}
- Unit 1 builds conceptual scaffolding for later representation and modeling units. {.fragment}

## 02. What “genomics” means (and what it does NOT mean)

- Analogy: map compositional/structural diversity to property phenotypes. {.fragment}
- Not literal biology transfer; it is an organizational discovery metaphor. {.fragment}
...
- Define design-space thinking and discovery-loop logic. {.fragment}
- Diagnose bias/leakage risks before model comparison. {.fragment}

## 04. Discovery bottleneck in classical materials workflows

- Classical trial-and-error is too slow for large combinatorial spaces. {.fragment}
- Data-driven prioritization reduces expensive experimental/DFT evaluations. {.fragment}

```{mermaid}
graph LR
    A[Design / Theory] --> B[Synthesis / Simulation]
    B --> C[Characterization / Testing]
...
- Digitization of simulation and instrumentation created reusable data assets. {.fragment}
- Opportunity comes with data governance and quality burdens. {.fragment}

## 06. Where this course connects to MFML and ML-PC

- MFML supplies formal risk/validation language. {.fragment}
- ML-PC supplies experimental-data cautionary patterns complementary to MG. {.fragment}

## 07. 90-minute roadmap

- Motivation -> data assets -> representation assumptions -> validity -> exercise bridge. {.fragment}
- Focus on scientific reliability, not benchmark chasing. {.fragment}

## 08. Checkpoint prompt: “Where does ML add value here?”

- Ask where ML is additive versus redundant to existing pipelines. {.fragment}
- Force explicit statement of decision target and cost. {.fragment}
...
- Treat composition+structure space as high-dimensional manifold. {.fragment}
- Search requires representations that preserve relevant invariances. {.fragment}

## 10. PSPP graph for materials discovery

::: {.columns}
::: {.column width="50%"}
- **P**rocessing {.fragment}
- **S**tructure {.fragment}
- **P**roperties {.fragment}
- **P**erformance {.fragment}
:::

::: {.column width="50%"}
- Discovery links these four pillars. {.fragment}
- MG emphasizes structure/property edges and candidate ranking. {.fragment}
:::
:::
...
- Common targets: formation energy, hull distance, bandgap, moduli. {.fragment}
- Target semantics determine feasible model and metric choices. {.fragment}
...
- Physical anchoring {.fragment}
- High cost (Time/Compute) {.fragment}
:::

::: {.column width="50%"}
### Surrogate Modeling
- Data-driven approximation {.fragment}
- Massive throughput gains {.fragment}
- Best for screening/optimization {.fragment}
:::
:::

- Best practice: active loop with high-fidelity validation of top candidates. {.fragment}

## 13. Screening logic: rank then validate

- Rank by expected utility, not only predicted mean. {.fragment}
- Incorporate uncertainty to avoid overconfident exploitation. {.fragment}

## 14. Why uncertainty is required for candidate prioritization

- Without uncertainty, candidate prioritization is fragile under shift. {.fragment}
- Use uncertainty for exploration-exploitation balance. {.fragment}

## 15. Domain knowledge constraints

- Charge neutrality, stability, symmetry act as hard/soft constraints. {.fragment}
- Constraints reduce implausible regions and improve sample efficiency. {.fragment}

## 16. First-principles + data-driven hybrid strategy

- Combine first-principles computations with learned surrogates. {.fragment}
- Hybrid loops enable rapid hypothesis generation with physical anchoring. {.fragment}

## 17. Micro-case: when pure data fit fails physically

- Show failure where model learns dataset origin instead of chemistry. {.fragment}
- Use this to motivate robust split design. {.fragment}
...
```{mermaid}
graph TD
    A[Materials Databases] --> B[DFT-based]
    A --> C[Curation / Aggregation]
    B --> D[Materials Project]
    B --> E[OQMD]
    B --> F[AFLOW]
    C --> G[NOMAD]
    C --> H[Optimade]
...
- **Materials Genomics** operationalizes discovery as a data-driven design process. {.fragment}
- **Discovery loops** reduce costs by prioritizing candidates before expensive validation. {.fragment}
- **PSPP** (Processing, Structure, Property, Performance) is the central paradigm of discovery. {.fragment}
- **Surrogate models** trade some fidelity for massive throughput gains in screening. {.fragment}
- **Physical constraints** (symmetry, charge) reduce the search space and improve efficiency. {.fragment}
- **Uncertainty quantification** is required to avoid overconfident exploitation. {.fragment}
- **Data leakage** (e.g., polymorphs) is a major risk in materials dataset design. {.fragment}
- **Grouped splits** (e.g., by composition) are necessary to test true generalization. {.fragment}
- **Provenance** is critical for scientific trust and reproducibility of data-driven claims. {.fragment}
- **Decision rules** should explicitly incorporate both model predictions and costs. {.fragment}
