...
::: {.outline-box .fragment data-fragment-index="5"} 
### 
...
.outline-box.fragment[data-fragment-index="0"]:not(.current-fragment) h3::before { content: ""; }
.outline-box.fragment[data-fragment-index="0"].current-fragment h3::before {
  content: "Basics, History & Early Demonstrations";
  transition: all 0.8s ease;
}
.outline-box.fragment[data-fragment-index="1"]:not(.current-fragment) h3::before { content: ""; }
.outline-box.fragment[data-fragment-index="1"].current-fragment h3::before {
  content: "The 4D-STEM Acquisition Chain";
  transition: all 0.8s ease;
}
.outline-box.fragment[data-fragment-index="2"]:not(.current-fragment) h3::before { content: ""; }
.outline-box.fragment[data-fragment-index="2"].current-fragment h3::before {
  content: "Direct Methods & Contrast Transfer";
  transition: all 0.8s ease;
}
.outline-box.fragment[data-fragment-index="3"]:not(.current-fragment) h3::before { content: ""; }
.outline-box.fragment[data-fragment-index="3"].current-fragment h3::before {
  content: "Iterative Methods";
  transition: all 0.8s ease;
}
.outline-box.fragment[data-fragment-index="4"]:not(.current-fragment) h3::before { content: ""; }
.outline-box.fragment[data-fragment-index="4"].current-fragment h3::before {
  content: "Imaging Fields";
  transition: all 0.8s ease;
}
.outline-box.fragment[data-fragment-index="5"]:not(.current-fragment) h3::before { content: ""; }
.outline-box.fragment[data-fragment-index="5"].current-fragment h3::before {
  content: "Outlook: Dynamics & Disorder";
  transition: all 0.8s ease;
}
</style>
...
## The 4D-STEM Acquisition Chain: Overview

- Every ptychography experiment is a chain — the weakest link limits the reconstruction:

electron gun → condenser optics & corrector → **probe** → scan system → sample → **pixelated detector** → data pipeline → reconstruction

::: {.columns}
::: {.column width="50%" .fragment}
- **Microscope side** determines the physics:
  - probe wavefunction (aperture, aberrations, coherence)
  - scan geometry (step size, dwell time, distortions)
:::
::: {.column width="50%" .fragment}
- **Detector & data side** determines what we record:
  - dynamic range, speed, noise of the camera
  - data rates, formats, live feedback
:::
:::

::: {.callout-note .fragment}
This section: hardware choices you make *before* any algorithm sees the data
:::
...
## Probe Formation: From Aperture to Wavefunction

::: {.columns}
::: {.column width="55%"}
- The probe is shaped by the aperture and the lens aberrations:

$$
\psi(\vec{r}) = \mathcal{F}^{-1}\left[ A(\vec{k}) \, e^{-i\chi(\vec{k})} \right]
$$

- $A(\vec{k})$: top-hat aperture with semi-convergence angle $\alpha$
- $\chi(\vec{k})$: aberration function, lowest orders

$$
\chi(\vec{k}) = \pi \lambda k^2 \, \Delta f + \frac{\pi}{2} \lambda^3 k^4 C_s + \ldots
$$
:::
::: {.column width="45%" .fragment}
- Diffraction-limited probe size: $d \approx 0.61 \, \lambda / \alpha$
- Depth of focus: $\mathrm{DOF} \approx \lambda / \alpha^2$
- Large $\alpha$ → small probe, but thin optical sections
- Defocus $\Delta f$ is a *free parameter* in ptychography — used deliberately
:::
:::

::: {.callout-note .fragment}
Ptychography reconstructs the probe → residual aberrations become fit parameters, not resolution limits
:::

## Aberration Correction: How Perfect Does the Probe Need to Be?

::: {.columns}
::: {.column width="55%"}
- Hardware correctors [@Haider_1998] cancel $C_s$ (and higher orders) → routine sub-Å probes at $\alpha \approx$ 20–35 mrad
- Remaining limits: chromatic aberration, residual higher-order aberrations, corrector stability
- For conventional STEM imaging: probe quality **is** the resolution
:::
::: {.column width="45%" .fragment}
- For ptychography the requirements relax:
  - probe is reconstructed alongside the object
  - defocused / structured probes are usable — even beneficial for dose spreading
  - super-resolution beyond the aperture limit is possible
:::
:::

::: {.callout-note .fragment}
Rule of thumb: spend your alignment time on *stability and coherence*, not on the last picometer of probe size
:::

## The Scan System: Dwell Time, Distortions, Drift

::: {.columns}
::: {.column width="55%"}
- Scan coils position the probe on a (usually square) grid
- Real scans are imperfect:
  - **flyback error** at line starts (hysteresis)
  - low-frequency **drift** (thermal, stage)
  - high-frequency jitter (interference, EMI)
- Errors of a few 10 pm already matter at atomic resolution
:::
::: {.column width="45%" .fragment}
- Practical mitigations:
  - fast scans + many frames instead of one slow scan
  - non-rectangular / spiral scan patterns
  - **post-facto position refinement** in the reconstruction (→ iterative methods section)
:::
:::

::: {.callout-note .fragment}
Scan positions are nuisance parameters: hardware gets you close, the algorithm does the rest
:::

## Detector Evolution: From Scintillators to Direct Detection

- Conventional detection: scintillator + fiber optics + CCD → slow (ms readout), noisy conversion, blooming
- **Direct electron detection** (electrons hit silicon directly) changed the game [@Levin_2021; @McMullan_2014]:
  - single-electron sensitivity → counting → Poisson-limited detection
  - kHz–MHz readout instead of Hz–kHz
- Two main architectures for 4D-STEM today:

::: {.columns}
::: {.column width="50%" .fragment}
**Monolithic Active Pixel Sensors (MAPS)**: thin sensor + integrated readout; many small pixels
:::
::: {.column width="50%" .fragment}
**Hybrid Pixel Array Detectors (PAD)**: thick sensor bump-bonded to a readout ASIC; fewer, smarter pixels
:::
:::

::: {.callout-note .fragment}
4D-STEM = one full diffraction pattern per scan position — the detector defines what is possible
:::
...
::: {.columns}
::: {.column width="55%"}
- Examples: Gatan K2/K3, Direct Electron DE-series, 4D Camera [@Ercius_2024]
- Thin, back-thinned CMOS sensor → sharp point-spread, electron counting
- Large pixel counts (1k–4k)², excellent DQE at low dose
:::
::: {.column width="45%" .fragment}
- Trade-offs for 4D-STEM:
  - limited count rate per pixel → beam current capped
  - central (bright-field) disk easily saturates
  - frame rate historically the bottleneck — solved by massive parallel readout (4D Camera: 87 kHz)
:::
:::

::: {.callout-note .fragment}
MAPS shine for low-dose, finely-sampled diffraction — beam-sensitive samples
:::
...
## Detector Metrics That Matter

<div style="font-size:0.8em">

| Metric | Meaning | Why it matters for ptychography |
|--------|---------|--------------------------------|
| DQE(ω) | How much of the signal's information survives detection | Dose efficiency — critical for beam-sensitive samples |
| MTF | Blurring of a single electron event across pixels | Mixes adjacent detector pixels → acts like partial coherence |
| Dynamic range | Brightest vs. faintest usable signal per pattern | Central disk is 10³–10⁶ × brighter than dark-field |
| Frame rate / event rate | Patterns (or events) per second | Sets total field of view per unit time; drift window |
| Duty cycle | Fraction of time actually recording | Dead time = wasted dose |
</div>

::: {.callout-note .fragment}
Counting detectors reach DQE close to the Poisson limit — integrating detectors add read noise to every pattern
:::

## Which Detector for Which Experiment?

<div style="font-size:0.75em">

| | MAPS (K3, 4D Camera) | Hybrid PAD (EMPAD, Merlin) | Event-driven (Timepix3/4) |
|---|---|---|---|
| Pixels | (1k–4k)² | (128–512)² | (256–512)² |
| Speed | up to 87 kHz frames | ~1–10 kHz frames | sparse, no frames |
| Dynamic range | limited (saturation) | up to 1:10⁶ | counting, per-event |
| Central beam | attenuate/avoid | fine | fine at low current |
| Best at | low dose, fine diffraction sampling | quantitative ptychography/DPC | ultrafast, in-situ, ultra-low dose |
</div>

::: {.callout-note .fragment}
There is no universally best camera — match dynamic range, speed and sampling to *your* reconstruction method
:::
...
## Live Processing: Closing the Loop

::: {.columns}
::: {.column width="55%"}
- Open-source streaming frameworks: **LiberTEM** [@Clausen_2020] — distributed, camera-agnostic live 4D-STEM processing
- Live virtual detectors, first-moment/DPC and even live SSB ptychography during acquisition [@Strauch_2021]
- GPU pipelines reconstruct while the stage is still there → immediate quality feedback
:::
::: {.column width="45%" .fragment}
- Why it matters:
  - catch bad alignment / sampling *before* the sample is burned
  - enables automated experiments: acquisition parameters adjusted on the fly
  - direct methods (WPO/SSB, next section) are fast enough to run live
:::
:::

::: {.callout-note .fragment}
Design the experiment with the reconstruction in mind — live feedback turns the microscope into a closed-loop instrument
:::
...
## First-Moment Imaging: Center of Mass of the Diffraction Pattern

::: {.columns}
::: {.column width="55%"}
- The mean momentum transfer at scan position $\vec{\rho}$:

$$
\langle \vec{k} \rangle (\vec{\rho}) = \frac{\int \vec{k} \; I(\vec{k}; \vec{\rho}) \, d\vec{k}}{\int I(\vec{k}; \vec{\rho}) \, d\vec{k}}
$$

- For a thin object, quantum mechanically exact [@Mueller_2014; @MuellerCaspary_2017]:

$$
\langle \vec{k} \rangle (\vec{\rho}) \propto \left[ |\psi|^2 \otimes \nabla_\perp V \right](\vec{\rho})
$$

→ the probe-smeared **projected electric field**
:::
::: {.column width="45%" .fragment}
- Segmented detectors (4 quadrants): classic DPC — fast but approximate
- Pixelated detectors: true first moment — quantitative
- Live first-moment imaging is routine [@Strauch_2021]
:::
:::

::: {.callout-note .fragment}
CoM imaging is the "free" field measurement inside every 4D-STEM dataset
:::

## From Momentum Transfer to E-Field and Charge Density

::: {.columns}
::: {.column width="55%"}
- Field: $\vec{E}_\perp \propto -\nabla_\perp V$ → directly from CoM
- Charge density via Gauss's law:

$$
\rho(\vec{r}) = \varepsilon_0 \, \nabla \cdot \vec{E}(\vec{r})
$$

- Potential by integration: **iDPC / iCoM** [@Lazic_2016] — light elements next to heavy ones become visible
:::
::: {.column width="45%" .fragment}
- Divergence doubles the noise sensitivity → needs high dose or heavy regularization
- All quantities are *projected* along the beam and convolved with $|\psi|^2$
:::
:::

::: {.callout-note .fragment}
One 4D dataset → field, charge, and potential maps, each with different noise behavior
:::

## Where the Simple Field Interpretation Breaks Down

::: {.columns}
::: {.column width="55%"}
- The linear CoM ↔ field relation assumes a **thin** object
- In real samples [@MuellerCaspary_2017]:
  - dynamical scattering & probe channeling re-distribute intensity
  - CoM no longer ∝ field beyond a few nm thickness
  - contrast can even *invert* with thickness
:::
::: {.column width="45%" .fragment}
- Mitigations:
  - keep samples thin; compare with multislice simulations
  - unit-cell averaging to boost SNR while watching for artifacts
  - or: use a reconstruction that *models* multiple scattering → ptychography
:::
:::

::: {.callout-note .fragment}
DPC/CoM is quantitative only where the weak-and-thin assumptions hold — always sanity-check with simulation
:::

## Ptychography as a Quantitative Field Probe

::: {.columns}
::: {.column width="55%"}
- Ptychography reconstructs the complex object $O(\vec{r}) = e^{i\sigma V(\vec{r})}$ → phase ∝ projected potential
- Advantages over CoM/DPC:
  - post-corrects aberrations, reconstructs the probe
  - dose-efficient use of *all* detector pixels
  - multislice variants handle thickness (→ MSP section)
- Deep sub-Å resolution demonstrated [@Jiang_2018]
:::
::: {.column width="45%" .fragment}
- The reconstructed potential is quantitative → differentiate once for $\vec{E}$, twice (Poisson) for $\rho$
- Noise is concentrated at low spatial frequencies — regularize accordingly
:::
:::

::: {.callout-note .fragment}
Ptychography turns "contrast" into a *measurement* of the potential with error bars
:::

## Charge Density and Bonding Electrons

::: {.columns}
::: {.column width="55%"}
- From the measured field/potential to charge density by Poisson's equation:

$$
\rho(\vec{r}) = -\varepsilon_0 \nabla^2 V(\vec{r})
$$

- Demonstrated at sub-Å resolution with 4D-STEM first-moment imaging [@Gao_2019]
- Subtract the independent-atom model (IAM) → **valence charge redistribution**: bonds, charge transfer
:::
::: {.column width="45%" .fragment}
![Placeholder — Fig. from @Gao_2019 (Nature 575, 480): real-space charge density of SrTiO₃ from 4D-STEM. Drop in licensed figure or own reconstruction.](./images/placeholder_charge_density.png){height="450px"}
:::
:::
...
::: {.callout-note .fragment}
Answering the outlook question of the MC2025 talk: yes — valence electron distributions are measurable
:::
...
## Lorentz Electron Ptychography

::: {.columns}
::: {.column width="55%"}
- Ptychography in field-free (Lorentz) mode [@Chen_2022_Lorentz]:
  - reconstructs magnetic phase of skyrmion lattices
  - resolution *beyond* the (poor) Lorentz-mode diffraction limit — superresolution
  - separates sample phase from probe artifacts
:::
::: {.column width="45%" .fragment}
![Placeholder — Fig. from @Chen_2022_Lorentz (Nat. Nanotechnol. 17, 1165): magnetic textures reconstructed by Lorentz ptychography. Drop in licensed figure.](./images/placeholder_lorentz_ptycho.png){height="450px"}
:::
:::
...
::: {.callout-note .fragment}
The same inverse-problem machinery from the iterative-methods section — only the physics in the forward model changes
:::

## Atomic-Scale Magnetic Fields — and Separating E from B

::: {.columns}
::: {.column width="55%"}
- Milestone: direct real-space imaging of the intrinsic magnetic field of an antiferromagnet (hematite) at atomic planes [@Kohno_2022]
- Key ingredients:
  - aberration-corrected magnetic-field-free optics
  - massive unit-cell averaging for SNR
  - subtracting the (much larger) electrostatic CoM signal
:::
::: {.column width="45%" .fragment}
- E/B separation strategies:
  - time reversal: flip the sample → $\vec{B}$ signal flips, $\vec{E}$ doesn't
  - compare against simulated electrostatic-only reference
:::
:::

::: {.callout-note .fragment}
Field imaging has matured from qualitative contrast to quantitative measurement — the second outlook question answered
:::
