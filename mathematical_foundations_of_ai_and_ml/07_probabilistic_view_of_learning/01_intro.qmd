...
::: {.notes}
- Keep this tight — it's a one-idea slide: sample below $2 f_{\max}$ and high frequencies don't vanish, they **masquerade** as low frequencies (aliasing). Draw the wagon-wheel / strobe analogy on the chalkboard if time.
- The ML-specific danger to emphasize: an aliased dataset contains *phantom low-frequency patterns* that look real. A flexible model will fit them confidently — you get a low training loss and a model that has learned an artifact of your sampling, not physics. This is a silent failure (no error is thrown).
- Materials tie-in: spectra, diffraction, time-resolved process signals — all are sampled; under-sampled XRD/spectroscopy is a real way to manufacture fake "features." One sentence is enough.
:::
