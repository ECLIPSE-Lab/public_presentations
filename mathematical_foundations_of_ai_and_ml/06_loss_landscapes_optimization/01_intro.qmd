...
- Core update rule: \\(\theta^{(t+1)} = \theta^{(t)} - \eta \nabla_\theta J(\theta^{(t)})\\)
...
- interpret the Hessian matrix and its eigenvalues as curvature descriptors,
...
- The cost function \\(J(\theta)\\) defines a surface over the parameter space \\(\mathbb{R}^p\\).
...
- Classification requires second-order information: the **Hessian matrix**.
- Minimum, maximum, or saddle point depends on the sign pattern of Hessian eigenvalues.
...
- \\(H\\) is symmetric for smooth loss functions (Schwarz's theorem).
...
- The condition number is defined as \\(\kappa(H) = \lambda_{\max} / \lambda_{\min}\\).
- \\(\kappa \approx 1\\): well-conditioned, isotropic curvature — gradient descent works well.
- \\(\kappa \gg 1\\): ill-conditioned — gradient descent oscillates in steep directions and crawls in flat ones.
...
- For a critical point to be a local minimum, **all** Hessian eigenvalues must be positive.
...
- Near a saddle point, the gradient is small in all directions — training slows dramatically.
...
- Plateaus are extended flat regions where \\(\|\nabla J\| \approx 0\\).
...
- **Ill-conditioning**: oscillation + slow convergence; diagnosed by large \\(\kappa(H)\\).
- **Saddle points**: near-zero gradient in all directions; escape requires curvature exploitation.
...
- Think of the parameter vector as a ball rolling on the loss surface.
- The ball accumulates **velocity** in directions of consistent gradient.
...
- Velocity update: \\(v^{(t+1)} = \alpha v^{(t)} - \eta \nabla J(\theta^{(t)})\\)
- Parameter update: \\(\theta^{(t+1)} = \theta^{(t)} + v^{(t+1)}\\)
...
- Key idea: evaluate the gradient at the **look-ahead** position \\(\theta + \alpha v\\).
...
- Different parameters experience different curvatures (different Hessian eigenvalues).
...
- Accumulator: \\(G_t = G_{t-1} + g_t^2\\) (element-wise).
- Update: \\(\theta_{t+1} = \theta_t - \frac{\eta}{\sqrt{G_t} + \epsilon} g_t\\)
...
- **Weakness**: \\(G_t\\) only grows, so effective learning rate monotonically decreases.
- Eventually, the learning rate becomes too small and training stops prematurely.

## 34. RMSProp: exponential moving average fix

- Replace the sum with a decaying average: \\(E[g^2]_t = \gamma E[g^2]_{t-1} + (1-\gamma)g_t^2\\).
...
- Update: \\(\theta_{t+1} = \theta_t - \frac{\eta}{\sqrt{E[g^2]_t} + \epsilon} g_t\\)
...
- Flatness can be quantified by the trace or top eigenvalues of the Hessian at the minimum.
...
1. The Hessian eigenvalues determine curvature magnitude in each direction.
2. The condition number \\(\kappa = \lambda_{\max}/\lambda_{\min}\\) measures landscape difficulty.
3. Saddle points vastly outnumber local minima in high-dimensional spaces.
4. Momentum accumulates velocity to dampen oscillations and accelerate convergence.
