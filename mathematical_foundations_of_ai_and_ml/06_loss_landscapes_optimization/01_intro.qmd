...
:::: {.notes}
This is the exam-weight slide of the geometry block. Each eigenvalue $\lambda_i$ is the curvature of a 1D parabola along eigenvector $\mathbf{v}_i$; the sign and magnitude of the spectrum *is* the local geometry. The one-line derivation worth the board: GD on a quadratic decouples in the eigenbasis into independent scalar recursions $\delta_i^{(t+1)} = (1-\eta\lambda_i)\,\delta_i^{(t)}$. Stability needs $|1-\eta\lambda_i|<1$, i.e. $\eta < 2/\lambda_{\max}$; convergence along the slowest direction then contracts at rate $1-\eta\lambda_{\min}$. Plug in the best stable $\eta\approx 2/\lambda_{\max}$ and the slow-direction rate is $\propto 1 - 2\lambda_{\min}/\lambda_{\max} = 1 - 2/\kappa$ with condition number $\kappa=\lambda_{\max}/\lambda_{\min}$. That single fact — *convergence speed is governed by $\kappa$, set by the eigenvalue spread* — is the spine of the entire conditioning/momentum/Adam arc.
