...
- The forward pass computes the prediction \\(\hat{y}\\) and stores all intermediate values.

## 13. Forward pass notation

- Pre-activation: \\(z^{(\ell)} = W^{(\ell)} a^{(\ell-1)} + b^{(\ell)}\\)
- Activation: \\(a^{(\ell)} = \sigma(z^{(\ell)})\\)
- Input layer: \\(a^{(0)} = \mathbf{x}\\)
- Output: \\(\hat{y} = a^{(L)}\\) after \\(L\\) layers.

## 14. Worked example: 2-layer network forward pass

- Architecture: 2 inputs, 2 hidden units, 1 output.
- Given: \\(\mathbf{x} = [1, 2]^T\\), weights \\(W^{(1)}, W^{(2)}\\), biases \\(b^{(1)}, b^{(2)}\\).
- Step 1: \\(z^{(1)} = W^{(1)}\mathbf{x} + b^{(1)}\\), \\(a^{(1)} = \sigma(z^{(1)})\\).
- Step 2: \\(z^{(2)} = W^{(2)}a^{(1)} + b^{(2)}\\), \\(\hat{y} = a^{(2)}\\).
- All intermediate values \\(z^{(\ell)}, a^{(\ell)}\\) are stored for the backward pass.

## 15. Why store intermediate activations?

- The backward pass requires \\(a^{(\ell-1)}\\) and \\(\sigma'(z^{(\ell)})\\) at every layer.
...
$$
\frac{\partial J}{\partial w_{ok}} = \delta_o \cdot a_k^{(L-1)}
$$
...
$$
\frac{\partial J}{\partial w_{kj}} = \delta_k \cdot a_j^{(\ell-1)}
$$

where:

$$
\delta_k = \sigma'(z_k) \cdot \sum_m w_{mk}^{(\ell+1)} \delta_m^{(\ell+1)}
$$
...
1. **Forward pass**: compute and store all \\(z^{(\ell)}, a^{(\ell)}\\) for \\(\ell = 1, \dots, L\\).
2. **Output delta**: \\(\delta^{(L)} = \nabla_a L \odot \sigma'(z^{(L)})\\).
3. **Backward loop**: for \\(\ell = L-1, \dots, 1\\):
   - \\(\delta^{(\ell)} = \sigma'(z^{(\ell)}) \odot \big((W^{(\ell+1)})^T \delta^{(\ell+1)}\big)\\)
...
- Compute \\(\delta^{(2)}\\) from the loss and output activation derivative.
- Propagate to \\(\delta^{(1)}\\) using the hidden-to-output weights and hidden activation derivatives.
...
$$
\frac{\partial J}{\partial a^{(\ell)}} = \prod_{m=\ell}^{L} \text{diag}(\sigma'(z^{(m)})) \cdot W^{(m+1)} \cdot \frac{\partial J}{\partial a^{(L+1)}}
$$
...
- If \\(\|W^{(\ell)}\|\\) is large, the gradient product grows exponentially:

$$
\prod_{m=1}^{L} \|W^{(m)}\| \cdot \|\sigma'(z^{(m)})\| \to \infty
$$
...
- **Gradient clipping**: cap \\(\|\nabla J\|\\) at a threshold \\(\tau\\):

$$
\nabla J \leftarrow \frac{\tau}{\|\nabla J\|} \nabla J \quad \text{if } \|\nabla J\| > \tau
$$
...
- The Jacobian of layer \\(\ell\\): \\(J^{(\ell)} = \frac{\partial a^{(\ell)}}{\partial a^{(\ell-1)}}\\).
- **Singular values** of \\(J^{(\ell)}\\) control gradient magnitude:
...
- **Question**: A 10-layer network uses sigmoid activations and weights initialized with \\(\|W^{(\ell)}\| \approx 1\\). What happens to the gradient at layer 1?

- A) Gradient grows exponentially.
- B) Gradient stays approximately constant.
- C) Gradient shrinks exponentially.
- D) Gradient oscillates unpredictably.

- **Answer**: C — sigmoid derivative \\(\leq 0.25\\), so \\(0.25^{10} \approx 10^{-6}\\).
...
- During training, plot \\(\|\nabla_{W^{(\ell)}} J\|\\) for each layer \\(\ell\\) over epochs.
