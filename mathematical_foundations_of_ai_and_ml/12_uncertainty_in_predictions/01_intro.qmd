...
$$
\text{Var}[y^*] = \underbrace{\mathbb{E}_\theta[\sigma^2(\theta)]}_{\text{aleatory}} + \underbrace{\text{Var}_\theta[\mu(\theta)]}_{\text{epistemic}}
$$
...
|----------|:------:|:-----------:|:----:|
| MLE/MAP | Single \\(\hat{y}\\) | None (or ad-hoc) | Low |
| Bayesian (exact) | Full \\(p(y^*|x^*,\mathcal{D})\\) | Principled | High |
...
- A **GP** is a distribution over functions: \\(f \sim \mathcal{GP}(m(x), k(x, x'))\\).
- Any finite collection of function values \\([f(x_1), \dots, f(x_N)]\\) is jointly Gaussian.
- The GP is fully specified by its **mean function** \\(m(x)\\) and **kernel function** \\(k(x, x')\\).
...
- The kernel function \\(k(x, x')\\) plays the role of the covariance matrix.
- This is a Bayesian nonparametric model — complexity grows with data [@murphy2012machine].

## 19. Mean function m(x)

- \\(m(x) = \mathbb{E}[f(x)]\\): the expected function value at each input.
- Common choice: \\(m(x) = 0\\) (zero-mean prior).
- Can encode prior knowledge: \\(m(x) = ax + b\\) for a linear trend.
- The mean function is updated to the posterior mean after observing data.

## 20. Kernel (covariance) function k(x, x')

- \\(k(x, x') = \text{Cov}[f(x), f(x')]\\): encodes the **correlation** between function values.
...
- **Matérn**: \\(k(x,x') \propto\\) Bessel function — adjustable smoothness via parameter \\(\nu\\).
- **Periodic**: captures repeating patterns.
- **Linear**: \\(k(x,x') = \sigma^2 x^\top x'\\) — equivalent to Bayesian linear regression.
...
$$
\mu^*(x^*) = \mathbf{k}_*^\top (K + \sigma_n^2 I)^{-1} \mathbf{y}
$$

$$
\sigma^{*2}(x^*) = k(x^*, x^*) - \mathbf{k}_*^\top (K + \sigma_n^2 I)^{-1} \mathbf{k}_*
$$

- \\(K\\): kernel matrix \\([k(x_i, x_j)]_{N \times N}\\). \\(\mathbf{k}_*\\): vector \\([k(x^*, x_i)]\\).
- The key operation is inverting \\((K + \sigma_n^2 I)\\) — cost \\(O(N^3)\\).

## 26. GP posterior: interpretation

- **Mean** \\(\mu^*(x^*)\\): best prediction — a weighted combination of training outputs.
- **Variance** \\(\sigma^{*2}(x^*)\\):
...
- Plot \\(\mu(x) \pm 2\sigma(x)\\): the **95% credible band**.
...
|:---------:|:----------:|
| Principled uncertainty quantification | \\(O(N^3)\\) training cost |
| Automatic complexity control (evidence) | Kernel design requires domain knowledge |
| Interpretable hyperparameters | Gaussian assumption may be limiting |
| Works well with small data | Scales poorly to high-dimensional inputs |
...
- A standard NN outputs a single \\(\hat{y}\\). An MDN outputs parameters of a **mixture of Gaussians**:

$$
p(y|x) = \sum_{k=1}^{K} \pi_k(x) \, \mathcal{N}(y | \mu_k(x), \sigma_k^2(x))
$$

- The network predicts mixing coefficients, means, and variances — all functions of input \\(x\\) [@neuer2024machine].
...
- Add noise to inputs during prediction: \\(\tilde{x} = x + \epsilon\\), \\(\epsilon \sim \mathcal{N}(0, \sigma_\epsilon^2)\\).
