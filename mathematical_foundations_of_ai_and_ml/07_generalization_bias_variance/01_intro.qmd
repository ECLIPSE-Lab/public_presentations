...
```{ojs}
//| echo: false
d3_reg = require("d3@7", "d3-regression@1")
seed = 42
randomVal = d3_reg.randomNormal.source(d3_reg.randomLcg(seed))(0, 0.2)
trueFunc = (x) => Math.sin(1.5 * Math.PI * x)

points = {
  const train = Array.from({length: 15}, (_, i) => {
    let x = (i+0.5) / 15 + (d3_reg.randomUniform.source(d3_reg.randomLcg(seed+i))() - 0.5) * 0.05;
...
polyModel = {
  try {
    return d3_reg.regressionPoly().x(d => d.x).y(d => d.y).degree(polyDegree)(trainPts);
  } catch(e) {
...
mse = {
  const trainMSE = d3_reg.mean(trainPts, d => Math.pow(polyModel.predict(d.x) - d.y, 2));
  const testMSE = d3_reg.mean(testPts, d => Math.pow(polyModel.predict(d.x) - d.y, 2));
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.line(d3_reg.range(0, 1.01, 0.01), {x: d => d, y: d => trueFunc(d), stroke: "gray", strokeDasharray: "4,4", strokeWidth: 2, title: "True Function"}),
    Plot.line(d3_reg.range(0, 1.01, 0.01).map(x => ({x: x, y: polyModel.predict(x)})), {x: "x", y: "y", stroke: "orange", strokeWidth: 4, title: "Fitted Model", clip: true})
  ]
})
```
:::
...
bvDatasets = {
  ...
  for(let i=0; i<numSamples; i++) {
    const rng = d3_reg.randomNormal.source(d3_reg.randomLcg(seed * 10 + i))(0, 0.4);
    const pts = Array.from({length: 12}, (_, j) => {
      let x = (j+0.5)/12 + (d3_reg.randomUniform.source(d3_reg.randomLcg(seed * 100 + i*12 + j))() - 0.5) * 0.05;
...
bvModels = bvDatasets.map(pts => d3_reg.regressionPoly().x(d=>d.x).y(d=>d.y).degree(bvDegree)(pts))

bvLines = bvModels.flatMap((model, i) => {
  return d3_reg.range(0, 1.05, 0.05).map(x => ({x: x, y: model.predict(x), lineId: i, type: "individual"}));
})

bvAverageLine = {
  const xs = d3_reg.range(0, 1.05, 0.05);
  return xs.map(x => {
...
Plot.plot({
  ...
  x: {domain: [0, 1], grid: true},
  marks: [
    Plot.line(d3_reg.range(0, 1.01, 0.01), {x: d => d, y: d => trueFunc(d), stroke: "gray", strokeDasharray: "4,4", strokeWidth: 3, title: "True Function"}),
...
constraintRegion = {
  if (regType === "Ridge (L2)") {
    return d3_reg.range(0, 2 * Math.PI + 0.1, 0.1).map(theta => {
      const r = constraintT;
