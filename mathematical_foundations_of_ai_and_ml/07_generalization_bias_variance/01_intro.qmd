...
::::
:::: {.column width="45%"}
![Polynomial fits of degree M = 0, 1, 3, 9 to N = 10 noisy points. The degree-9 fit is a textbook example of overfitting (Bishop Fig. 1.4).](images/bishop_poly_degree_sweep.png){width=100%}
::::
::::
...
![Train vs test RMS error as a function of polynomial order M. Low M → underfitting; M = 9 → test error explodes (Bishop Fig. 1.5).](images/bishop_train_test_rms_vs_degree.png){width=90%}
::::
:::: {.column width="45%"}
![Overfitting boundary in a classification setting (Neuer Fig. 4.8): the overfit boundary memorizes noise and fails to generalize.](images/neuer_overfitting_classification.png){width=95%}
::::
::::
...
::::
:::: {.column width="45%"}
![High-bias (top) vs high-variance (bottom) regime visualised by 20 fitted curves and their mean (red) vs the true function (green). (Murphy Fig. 6.5, based on Bishop Fig. 3.5.)](images/murphy_bias_variance_ridge_illustration.png){width=100%}
::::
::::

## Visual: U-shaped total error curve — from data

![Bias², variance, and their sum vs regularization strength ln(λ). The minimum of Bias² + Variance (≈ ln λ = −0.31) closely matches the test-set error minimum — the operational sweet spot (Bishop Fig. 3.6).](images/bishop_bias_variance_tradeoff_curve.png){width=60% fig-align="center"}
...
::::
:::: {.column width="45%"}
![L = 100 datasets fitted with different regularization strengths. Left column: individual fits. Right column: ensemble mean vs true sinusoid. High λ → low variance, high bias (top); low λ → high variance, low bias (bottom). (Bishop Fig. 3.5)](images/bishop_bias_variance_fitted_curves.png){width=100%}
::::
::::
...
::::
:::: {.column width="45%"}
![Degree-14 polynomial with increasing L2 regularization. Left (λ ≈ 0): wiggly overfit. Right (large λ): over-smoothed. Stars mark the same λ values as in Figure 7.8. (Murphy Fig. 7.7)](images/murphy_poly_regularization_fit.png){width=100%}
::::
::::
...
::::
:::: {.column width="45%"}
![Effect of regularization strength on an M = 9 polynomial. ln λ = −18 (left) suppresses overfitting; ln λ = 0 (right) over-shrinks. (Bishop Fig. 1.7)](images/bishop_regularization_lambda_effect.png){width=100%}

![McClarren Fig. 2.7: Ridge circular constraint. The LS unconstrained optimum is outside the circle; the ridge solution (star) lands on the boundary.](images/mcclarren_ridge_geometric_view.png){width=100%}
::::
::::
...
::::
:::: {.column width="45%"}
![Ridge (circle, left) vs Lasso (diamond, right) constraint regions with OLS error ellipses. The lasso solution lands at a corner, setting w₁* = 0 exactly. (Bishop Fig. 3.4)](images/bishop_ridge_lasso_constraint_regions.png){width=100%}
::::
::::
...
::::
:::: {.column width="50%"}
![McClarren Fig. 2.8: Lasso diamond constraint. The elliptic error contours touch the diamond at a corner — one coefficient is exactly zero, producing a sparse solution.](images/mcclarren_lasso_diamond_geometric.png){width=95%}
::::
::::
...
::::
:::: {.column width="45%"}
![Train MSE (dashed blue) and test MSE (solid red) vs log(λ) for ridge-regularized degree-14 polynomial. Right of minimum = underfit. Left of minimum = overfit. (Murphy Fig. 7.8a)](images/murphy_train_test_mse_vs_lambda.png){width=100%}

![5-fold CV error vs log(λ) with one-SE rule (blue line) for selecting a parsimonious λ. (Murphy Fig. 6.6b)](images/murphy_cv_lambda_selection.png){width=100%}
::::
::::
