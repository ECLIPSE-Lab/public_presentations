...
- Empirical Risk Minimization: \\(\hat{\theta} = \arg\min_\theta \frac{1}{N}\sum_{i=1}^{N} L(f_\theta(x_i), y_i)\\).
...
- Consider the expected loss over both the training data \\(\mathcal{D}\\) and a new test point \\((x, y)\\):

$$
\text{EPE}(x) = \mathbb{E}_{\mathcal{D}} \mathbb{E}_{y|x} \big[ (y - \hat{f}_{\mathcal{D}}(x))^2 \big]
$$

- This averages over all possible training sets and all possible true outputs at \\(x\\).

## 12. Decomposing squared error — step 1

- Add and subtract the expected prediction \\(\mathbb{E}_{\mathcal{D}}[\hat{f}(x)]\\):

$$
y - \hat{f}(x) = \underbrace{(y - f(x))}_{\text{noise}} + \underbrace{(f(x) - \mathbb{E}_{\mathcal{D}}[\hat{f}(x)])}_{\text{bias}} + \underbrace{(\mathbb{E}_{\mathcal{D}}[\hat{f}(x)] - \hat{f}(x))}_{\text{variance term}}
$$

## 13. Decomposing squared error — step 2

- Square the expression and take expectations.
- Cross-terms vanish because noise is independent of the model and \\(\mathbb{E}_{\mathcal{D}}[\hat{f}(x) - \mathbb{E}_{\mathcal{D}}[\hat{f}(x)]] = 0\\).
- The three surviving terms give us the decomposition.

## 14. The three components

$$
\text{EPE}(x) = \underbrace{\sigma^2_{\text{noise}}}_{\text{irreducible}} + \underbrace{\big(\mathbb{E}_{\mathcal{D}}[\hat{f}(x)] - f(x)\big)^2}_{\text{Bias}^2} + \underbrace{\mathbb{E}_{\mathcal{D}}\big[(\hat{f}(x) - \mathbb{E}_{\mathcal{D}}[\hat{f}(x)])^2\big]}_{\text{Variance}}
$$
...
$$
J_{\text{reg}}(\theta) = \underbrace{\frac{1}{N}\sum_{i=1}^{N} L(\hat{y}_i, y_i)}_{\text{data fit}} + \underbrace{\lambda \cdot \Omega(\theta)}_{\text{complexity penalty}}
$$

- The penalty \\(\Omega(\theta)\\) grows with model complexity.
...
$$
\hat{\theta} = \arg\min_\theta \left[ R_N(\theta) + \lambda \, \Omega(\theta) \right]
$$
...
- Penalty: \\(\Omega(\theta) = \|\mathbf{w}\|_2^2 = \sum_j w_j^2\\).
- Loss:
...
- Penalty: \\(\Omega(\theta) = \|\mathbf{w}\|_1 = \sum_j |w_j|\\).
- Loss:
...
$$
\Omega(\theta) = \alpha \|\mathbf{w}\|_1 + (1 - \alpha) \|\mathbf{w}\|_2^2
$$
...
- **Training set**: used to fit model parameters \\(\theta\\).
