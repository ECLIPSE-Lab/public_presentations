...
::::
:::: {.column width="55%"}
![A simple autoencoder: inputs (green) are compressed to a small latent layer $z$ (blue) and reconstructed at the output (red). The latent bottleneck is the only constraint — it imposes no structure on what $z$ means. (McClarren 2023)](images/autoencoder_architecture.png){width=90%}
::::
::::
...
::::
:::: {.column width="50%"}
![Normal (blue) vs Cauchy/Student-t (orange) distribution with the same center. At $|x|=4$ the Cauchy value is ~140× larger than the Gaussian. This heavy tail is what allows moderate-distance pairs to remain separated in the 2-D embedding — solving the crowding problem. (McClarren 2023)](images/gaussian_vs_student_t_heavy_tail.png){width=100%}
::::
::::
