...
::: {.incremental}
- Apply **Optimization Algorithms** like Gradient Descent, Minibatch SGD, and Adam to traverse loss landscapes.
...
- Understand advanced optimization update rules.
...
::: {.incremental}
- **Optimization**: Minimizing the loss on the **training set**. The goal is simply to find parameters $\mathbf{w}$ that fit the observed data.
- **Generalization**: The model's ability to perform well and minimize the risk on **unseen data** (test set).
- **The Dilemma**: We can often optimize training loss to zero (perfect fit/overfitting), but that usually destroys generalization.
- Our primary tool for learning is optimization, but we must use it carefully (with regularization, early stopping) to ensure generalization.
:::
...
::: {.column width="50%"}
::: {.incremental}
- **Convex Functions**: Have a single, global minimum point. Any local minimum is guaranteed to be the global minimum.
  - E.g., MSE for Linear Regression.
- **Non-Convex Functions**: Have local minima, flat regions, and **saddle points**.
- **Saddle Points**: Zero gradient, but dips in one direction and rises in another. These stall optimization in high dimensions.
:::
:::
...
::: {.incremental}
- **Intuition**: To find the minimum of a valley, look at the slope (gradient) and take a step in the steepest downward direction.
- **Taylor Expansion**: $f(\mathbf{x} - \eta \nabla f(\mathbf{x})) \approx f(\mathbf{x}) - \eta \|\nabla f(\mathbf{x})\|^2$, which is strictly less than $f(\mathbf{x})$ for small $\eta > 0$.
- **Update Rule**: $$\mathbf{w}_{t+1} = \mathbf{w}_t - \eta \nabla f(\mathbf{w}_t)$$
- $\eta$ is the **Learning Rate**: Determines the step size.
  - Too small: Very slow convergence.
  - Too large: Overshoots the minimum and diverges.
:::

## Interactive: Learning Rate Dynamics

::: {.columns}
::: {.column width="30%"}
Adjust the learning rate $\eta$ and see the trajectory of Gradient Descent on a 2D convex bowl: $f(x, y) = 0.5x^2 + 2y^2$. Notice how the $y$-direction (steeper) causes oscillation when $\eta$ is too high.

```{ojs}
//| echo: false
viewof lr = Inputs.range([0.01, 1.1], {value: 0.1, step: 0.01, label: "Learning Rate (η)"})
```
:::
...
chart_gd = {
  const pts = [];
  let w_x = -4, w_y = 3;
  pts.push({x: w_x, y: w_y, iter: 0});
  
  for(let i=1; i<=20; i++) {
    let grad_x = w_x;     // df/dx = x
    let grad_y = 4 * w_y; // df/dy = 4y
    w_x = w_x - lr * grad_x;
    w_y = w_y - lr * grad_y;
    pts.push({x: w_x, y: w_y, iter: i});
    if(Math.abs(w_x) > 10 || Math.abs(w_y) > 10) break; // diverge bound
  }
  
  const contours = [];
  for(let c=0.5; c<=12; c+=2) {
    for(let t=0; t<Math.PI*2; t+=0.1) {
      contours.push({
        x: Math.sqrt(2*c) * Math.cos(t),
        y: Math.sqrt(c/2) * Math.sin(t),
        level: c
      });
    }
  }
  ...
  return Plot.plot({
    ...
    marks: [
      ...
      Plot.line(pts, {x: "x", y: "y", stroke: "red", strokeWidth: 2, markerEnd: "arrow"}),
      Plot.dot(pts, {x: "x", y: "y", fill: "red", r: 4}),
      Plot.dot([{x: 0, y: 0}], {x: "x", y: "y", fill: "blue", r: 5})
    ]
  });
}
```
...
::: {.incremental}
- **The Problem**: Standard GD computes the gradient over the **entire dataset** (size $n$). This is extremely slow per update: $\mathcal{O}(n)$.
- **Stochastic Shift**: Pick a **single** random data point $i$ and compute the gradient:
  $$\mathbf{w}_{t+1} = \mathbf{w}_t - \eta \nabla L_i(\mathbf{w}_t)$$
- **Why it works**: In expectation, the stochastic gradient is equal to the true full gradient: $\mathbb{E}_i[\nabla L_i] = \nabla f$.
- **Behavior**: It bounces randomly around the minimum but makes extremely fast initial progress.
:::

## Minibatch SGD

::: {.incremental}
- **Compromise**: Instead of 1 point (too noisy, poor hardware utilization) or $n$ points (too slow computation), we pick a **minibatch** of size $b$ (e.g., 32 to 256).
- **Update**:
  $$\mathbf{w}_{t+1} = \mathbf{w}_t - \frac{\eta}{b} \sum_{i \in \mathcal{B}_t} \nabla L_i(\mathbf{w}_t)$$
- **Advantages**:
  1. **Variance Reduction**: Averages out the noise of individual samples.
  2. **Vectorization**: Modern GPUs shine at matrix-matrix products. Processing $b$ samples in parallel is almost as fast as processing 1.
- **This is the workhorse of modern Deep Learning.**
:::

## Accelerating Convergence (Momentum & Adam)

::: {.incremental}
- **The Challenge**: Ill-conditioned loss functions (like narrow ravines) cause SGD to oscillate across the ravine and move sluggishly along the minimum.
- **Momentum**: Mimics physics. Add a "velocity" term to remember past gradients.
  $$\mathbf{v}_{t} = \beta \mathbf{v}_{t-1} + \nabla L(\mathbf{w}_{t})$$
  $$\mathbf{w}_{t+1} = \mathbf{w}_t - \eta \mathbf{v}_t$$
  - Smooths out oscillations and accelerates descent exactly where gradients point in the same direction consistently.
- **Adam (Adaptive Moment Estimation)**: Automatically adjusts the learning rate *per parameter* by keeping moving averages of both the gradient (1st moment) and the squared gradient (2nd moment).
:::
