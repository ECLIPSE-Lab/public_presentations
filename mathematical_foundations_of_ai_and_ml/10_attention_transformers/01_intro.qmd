---
title: |
  Mathematical Foundations of AI & ML<br>Unit 10: Attention & Transformers
...
::::
:::: {.column width="50%"}
![Basic recurrent neural network: the hidden state $z$ feeds back into itself at the next time step (McClarren 2022, Fig. 7.1).](images/rnn_basic.png){width=90%}
::::
::::
...
::::
:::: {.column width="55%"}
![Unrolled basic RNN across three time steps. Weights $w$, $v$, $w_o$ are shared across time (McClarren 2022, Fig. 7.2).](images/rnn_unrolled.png){width=100%}
::::
::::
...
::::
:::: {.column width="60%"}
![Three forms of recurrence: direct, lateral, and indirect feedback (Neuer 2023, Fig. 4.9).](images/rnn_feedback_types.png){width=100%}
::::
::::
...
::::
:::: {.column width="55%"}
![Complete LSTM cell at time $t$: state track $C$ (top horizontal line) and output track $z$ (diagonal), controlled by three gates (McClarren 2022, Fig. 7.8).](images/lstm_cell_complete.png){width=100%}
::::
::::
...
::::
:::: {.column width="60%"}
![LSTM signal flow: memory track $m$ and hidden state $h$ flow left to right, modulated by the three gates (Neuer 2023, Fig. 4.11).](images/lstm_cell_neuer.png){width=100%}
::::
::::
