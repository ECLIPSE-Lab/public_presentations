...
- Encoder \\(f_\phi: \mathbb{R}^{d_x} \to \mathbb{R}^{d_z}\\) maps each input to a latent code.
- The collection \\(\{z_i = f_\phi(x_i)\}\\) forms a **point cloud** in the latent space.
...
- When \\(d_z = 2\\), the latent space can be plotted directly.
...
- Minimize the KL divergence between high-dim (\\(P\\)) and low-dim (\\(Q\\)) similarities:

$$
\text{KL}(P \| Q) = \sum_{i \neq j} p_{ij} \log \frac{p_{ij}}{q_{ij}}
$$

- Optimize the embedding coordinates \\(\{y_i\}\\) via gradient descent.
...
10. Visualization methods (t-SNE, UMAP) require careful interpretation.
