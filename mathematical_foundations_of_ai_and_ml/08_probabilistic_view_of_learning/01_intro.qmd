...
### Visual Placeholder: Noise Augmentation
Diagram showing a single data point expanded into a "cloud" of points via noise injection, leading to a smoother, more robust decision boundary.
:::
...
::: {.callout-note icon=false}
### Visual Placeholder: MDN Output
Plot of a bimodal predictive distribution $p(y|x)$ where a standard network would only predict the (useless) mean between the two peaks.
:::
...
::: {.callout-note icon=false}
### Visual Placeholder: Process Corridor
2D heat map of sintering temperature vs density, with "safe" probability contours (corridors) highlighted in green.
:::
