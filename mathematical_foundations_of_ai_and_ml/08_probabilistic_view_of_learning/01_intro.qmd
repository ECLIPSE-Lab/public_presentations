...
- Unit 1: \\(\hat{\theta} = \arg\min_\theta \mathbb{E}_{(x,y) \sim P}[L(f_\theta(x), y)]\\).
...
- **Conditional**: condition on some variables — the result is Gaussian with updated mean and reduced covariance.
...
- Given observed data \\(\mathcal{D} = \{x_1, \dots, x_N\}\\) (assumed i.i.d.):

$$
L(\theta) = \prod_{i=1}^{N} p(x_i | \theta)
$$

- \\(L(\theta)\\) is a function of the **parameters** \\(\theta\\), not the data.
- It measures how well \\(\theta\\) explains the observed data.
...
- Maximizing \\(\ell(\theta)\\) gives the same solution as maximizing \\(L(\theta)\\).
...
- Set \\(\nabla_\theta \ell(\theta) = 0\\) and solve.
...
- For a regression model \\(y = f_\theta(x) + \epsilon\\) with \\(\epsilon \sim \mathcal{N}(0, \sigma^2)\\):

$$
\ell(\theta) = -\frac{N}{2}\log(2\pi\sigma^2) - \frac{1}{2\sigma^2}\sum_{i=1}^{N}(y_i - f_\theta(x_i))^2
$$

- Maximizing \\(\ell(\theta)\\) w.r.t. \\(\theta\\) is **equivalent** to minimizing MSE.
...
- **Posterior** \\(p(\theta|\mathcal{D})\\): what we believe about \\(\theta\\) after seeing data.
- **Likelihood** \\(p(\mathcal{D}|\theta)\\): how probable the data is under each \\(\theta\\).
...
- The **likelihood** is the same function used in MLE — it connects data to parameters.
...
- **Gaussian prior** \\(p(\theta) \propto \exp(-\frac{\lambda}{2}\|\theta\|_2^2)\\): MAP = **Ridge regression**.
- **Laplace prior** \\(p(\theta) \propto \exp(-\lambda\|\theta\|_1)\\): MAP = **Lasso regression**.
