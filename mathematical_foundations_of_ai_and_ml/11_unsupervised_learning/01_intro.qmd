...
## 03. Supervised vs unsupervised learning

- **Supervised**: given input-output pairs \\((x_i, y_i)\\) — learn the mapping.
- **Unsupervised**: given inputs \\(x_i\\) only — discover hidden structure.
- No teacher signal — the algorithm must find patterns on its own.
- Unsupervised learning is often a preprocessing step for supervised tasks.

## 04. Why unsupervised learning matters in engineering
...
## 05. Types of unsupervised learning
...
## 07. What defines a "good" cluster?
...
## 18. Checkpoint: K-Means failure mode
...
- In reality, a point between two clusters might plausibly belong to either.
- **Soft clustering**: assign each point a **probability** of belonging to each cluster.
...
- The parameters \\(\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k, \pi_k\\) are **coupled** through the responsibilities.
...
- Given current parameters \\(\theta^{(t)} = \{\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k, \pi_k\}\\):
...
1. **Initialize**: set \\(\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k, \pi_k\\) (e.g., from K-Means).
2. **Repeat until convergence**:
   - **E-step**: compute responsibilities \\(r_{ik}\\) for all \\(i, k\\).
   - **M-step**: update \\(\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k, \pi_k\\) using weighted statistics.
...
- The **auxiliary function** \\(Q(\theta | \theta^{(t)}) = \mathbb{E}_{z|x,\theta^{(t)}}[\log p(\mathbf{x}, z | \theta)]\\).
...
- **Answer**: The posterior probability of each cluster \\(k\\) given each data point \\(x_i\\), under the current model parameters. They quantify how much each cluster "explains" each point.
...
- Compression ratio = log₂(codebook size) bits per data point.
