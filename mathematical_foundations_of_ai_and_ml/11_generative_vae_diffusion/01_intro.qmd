---
title: |
  Mathematical Foundations of AI & ML<br>Unit 11: Generative Models — VAE & Diffusion
...
::::
:::: {.column width="45%"}
![Plate notation for a latent-variable generative model: open circle $z_i$ = hidden, shaded circle $x_i$ = observed. The plate (box labelled $N$) means the pattern repeats over all data points. Source: Bishop Ch. 8.](images/plate_notation_observed_latent.png){width=75%}
::::
::::
...
::::
:::: {.column width="45%"}
![The generative model as a plate DGM: $\pi$ mixes over component assignments $z_i$ (latent), which together with parameters produce observations $x_i$ (shaded). Source: Murphy Ch. 24.](images/mixture_model_plate_dgm.png){width=80%}
::::
::::
...
::::
:::: {.column width="45%"}
![A latent-variable generative model: factor analysers with latent $\boldsymbol{z}_i$ (open, hidden) and observed $\boldsymbol{x}_i$ (shaded). The parameters $\boldsymbol{\mu}_k, \boldsymbol{W}_k$ decode latents into observations. Source: Murphy Ch. 12.](images/latent_variable_model_plate.png){width=80%}
::::
::::
...
- The blurriness is the historical motivation for GANs, then for diffusion.
::::
...
::::
:::: {.column width="45%"}
![Metropolis–Hastings sampling from a 2D Gaussian: accepted steps (green) and rejected steps (red). Iterative refinement through many small moves — each step uses the current state. Source: Bishop Ch. 11.](images/metropolis_chain_steps.png){width=85%}
...
- Compare to GANs: no adversarial dynamics, no min-max optimization, just stable supervised learning.
::::
...
- Most common architecture: a **U-Net** with timestep embedding (sinusoidal).
...
::::
:::: {.column width="45%"}
![MH chain traces: (a) too-small proposal → chain trapped, one mode only; (b) too-large → sticky, high rejection; (c) right scale → good mixing. Diffusion faces the same step-size tradeoff — too few steps = bad quality, too many = slow. Source: Murphy Ch. 24.](images/mcmc_chain_trace_mh_proposals.png){width=95%}
::::
::::
...
- Diffusion models are formally **score-based generative models**.
::::
::::
:::: {.column width="45%"}
![Gibbs sampling on a correlated 2D Gaussian: each step updates one variable conditioned on the other, creating zig-zag moves of size $l$ (conditional SD) along a distribution of width $L$. Requires $O((L/l)^2)$ steps for an independent sample — analogous to why diffusion needs many steps. Source: Bishop Ch. 11.](images/gibbs_sampling_correlated_gaussian.png){width=85%}
::::
::::

:::: {.notes}
- The score-matching connection unifies diffusion with stochastic differential equations (Song et al. 2021).
- For students who go on to research: this is the deep theory; for everyone else, it is a pointer.
- The Gibbs figure illustrates iterative refinement in a correlated space — step size matters, and many small steps are needed for high-dimensional distributions.
::::
...
:::: {.notes}
- GANs (Goodfellow et al. 2014) dominated 2015-2021.
- Their fundamental issue is the min-max optimization; diffusion has a clean MSE.
::::
...
- Default for serious generation in 2026: diffusion. For fast latent exploration: VAE. For exact likelihoods: flow.
::::
