...
- Diffusion models are formally **score-based generative models** [@song2021scoresde].
::::
::::
:::: {.column width="50%"}
![Forward SDE (data $\to$ noise): $d\mathbf{x} = \mathbf{f}(\mathbf{x},t)\,dt + g(t)\,d\mathbf{w}$. Reverse SDE (noise $\to$ data): replace $\mathbf{f}$ with $\mathbf{f} - g^2\nabla_\mathbf{x}\log p_t(\mathbf{x})$ and run backwards. The score $\nabla_\mathbf{x}\log p_t(\mathbf{x})$ is learned by the denoiser. Source: [@song2021scoresde] Fig. 1 (arXiv 2011.13456).](images/score_sde_overview.png){width=100%}
::::
::::
...
- The SDE overview figure shows the clean duality: forward = add noise via SDE; reverse = denoise using score.
::::
