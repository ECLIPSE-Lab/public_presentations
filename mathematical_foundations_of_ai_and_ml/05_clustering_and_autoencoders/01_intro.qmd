...
::::
:::: {.column width="45%"}
![K-means iterations on the Old Faithful data (Bishop Fig. 9.1). Each row shows one E-step (assign) and one M-step (update centroids). Converges in 3 M-steps.](images/kmeans_lloyd_iterations.png){width=100%}
::::
::::
...
::::
:::: {.column width="45%"}
![Cost function $J$ after each E-step (blue) and M-step (red) for the Old Faithful example. Converged after 3 M-steps. (Bishop Fig. 9.2)](images/kmeans_cost_convergence.png){width=100%}
::::
::::
...
::::
:::: {.column width="55%"}
![Effect of linkage on the dendrogram for yeast gene-expression data: (a) single link — chaining artifacts; (b) complete link — compact clusters; (c) average link — a compromise. (Murphy Fig. 25.15)](images/dendrogram_linkage_comparison.png){width=100%}
::::
::::
...
::::
:::: {.column width="45%"}
![Same 500 points from a 3-Gaussian mixture: (a) complete data with hard labels, (b) unlabeled (incomplete data), (c) soft coloring by responsibilities $\gamma_{ik}$ — purple = uncertain. (Bishop Fig. 9.5)](images/gmm_hard_soft_responsibilities.png){width=100%}
::::
::::
...
::::
:::: {.column width="45%"}
![A mixture of 3 Gaussians in 2D: (a) contours of each component; (b) the combined density surface. Each component can have its own shape and orientation. (Murphy Fig. 11.3)](images/gmm_contours_surface.png){width=100%}
::::
::::
...
::::
:::: {.column width="45%"}
![EM for GMM on Old Faithful: (a) initial random parameters; (b) responsibilities after 1st E-step (purple = uncertain); (c)–(f) convergence over 16 iterations. (Murphy Fig. 11.11)](images/em_gmm_steps.png){width=100%}
::::
::::
...
::::
:::: {.column width="50%"}
![EM for a 2-component GMM on the Old Faithful data. (a) initialization; (b)–(f) E/M steps shown with soft color-coded responsibilities. Ellipses show the 1-$\sigma$ contours of each Gaussian. (Bishop Fig. 9.8)](images/em_gmm_iterations.png){width=100%}
::::
::::
...
::::
:::: {.column width="55%"}
![A deep autoencoder: (a) greedily pre-train RBM layers; (b) unroll into encoder/decoder by tying weights; (c) fine-tune end-to-end with backprop. The bottleneck (middle layer) is the latent code $z$. (Murphy Fig. 28.3)](images/autoencoder_architecture_training.png){width=100%}
::::
::::
...
::::
:::: {.column width="50%"}
![2D latent space of a deep autoencoder trained on Reuters news articles. Left: LSA (linear, overlapping topics). Right: deep AE (nonlinear, well-separated topic clusters). Labels were not used during training. (Murphy Fig. 28.5)](images/autoencoder_latent_space_2d.png){width=100%}
::::
::::
