...
::: {.fragment}
![Aleatoric (irreducible noise) vs epistemic (lack of knowledge) uncertainty: the epistemic part grows exactly where data is sparse.](images/uncertainty_types.png){width="40%" fig-align="center"}
:::
...
::: {.callout-note .fragment}
Small $n$, one smooth physical input, real cost per data point — the textbook regime for a GP.
:::
...
::: {.column width="45%"}
::: {.fragment}
![GP regression on sparse data: mean prediction with a 95% CI that narrows at the training points and widens between them.](images/gp_uncertainty.png){width="100%"}
:::
:::
:::

::: {.callout-note .fragment}
At $n \sim 30$ the GP's posterior CI is honest **by construction** — no separate calibration set needed.
:::
...
::: {.column width="40%"}
::: {.fragment}
![Sample functions drawn from a GP prior — the kernel sets their smoothness, amplitude, and correlation length before any data arrives.](images/gp-sample-prior-functions.svg){width="90%"}
:::
:::
:::
...
::: {.column width="45%"}
::: {.fragment}
![GP posterior samples with the 95% credible band — the ribbon collapses at observations and widens in the gaps.](images/gp-posterior-samples-95.svg){width="90%"}
:::
:::
:::

::: {.callout-note .fragment}
Trust the ribbon **only after** the leave-one-out reliability diagram says you may.
:::
...
::: {.column width="45%"}
::: {.fragment}
![GP point predictions with their credible band: the band balloons wherever data is absent — the honest answer.](images/gp-point-predictions.svg){width="90%"}
:::
:::
:::
...
::: {.columns}
::: {.column width="33%"}
![Iteration 0: one sample, flat GP surrogate, uncertainty everywhere.](images/active_learning_iter0.png){width="100%"}
:::
::: {.column width="33%"}
![Iteration 4: the mean tracks the ground truth near sampled points.](images/active_learning_iter4.png){width="100%"}
:::
::: {.column width="33%"}
![Iteration 8: surrogate converged — each query went where $\sigma$ was largest.](images/active_learning_iter8.png){width="100%"}
:::
:::
:::
...
## 23. Materials-acceleration-platform framing
...
## 24. Reliability diagrams on lab data
...
## 25. OOD detection — when the model sees something new
...
## 26. Recall: conformal coverage as the wrapper layer (MFML W7)
...
- **GP (§C) → split conformal.** GP CI is already smooth; wrap to convert the model-conditional ribbon into a frequentist coverage guarantee on the held-out lab batch.
- **MC Dropout / U-Net (§D) → split conformal per class** on the held-out tile set; gives a calibrated per-pixel set-valued prediction.
- **Heteroscedastic regression near a regime boundary (§E keyhole) → CQR.** Constant-width CP is wasteful where noise is locally regime-dependent.
:::
:::
::: {.column width="45%"}
::: {.fragment}
...
- Finite-sample guarantee on the $n \sim 30$ specimens of §C.
...
::: {.callout-warning .fragment}
**Materials-specific failure mode.** Tool drift (§D SEM #1 → #2) breaks exchangeability silently. Always re-run coverage on a per-tool calibration set, or accept that the guarantee is gone.
:::
:::
:::
...
## 28. What MFML W7 + W12 cover that we skipped
...
6. **The math lives in MFML W7 (conformal) + W12 (Bayes, GP, calibration).** ML-PC W13 is about **using** that math to save tests, find process windows, and ship trustworthy models.
:::
