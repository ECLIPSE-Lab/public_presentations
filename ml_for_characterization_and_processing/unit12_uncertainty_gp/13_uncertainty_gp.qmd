...
::: {.fragment}
- **Hands-on:** reproduce this entire case study in the [Week-13 notebook](https://eclipse-lab.github.io/Ai4MatLectures/notebooks/MLPC/week13_mcdropout_metaldam.html) — Colab-ready, data via `ai4mat.datasets.MetalDAMDataset`.
:::
:::
:::
...
:::
::: {.column width="47%"}
![Measured on the held-out MetalDAM micrographs: fully automatic recovers **82.9%** of defect pixels; entropy-ranked review adds **+1.4 points at 1%** review and **+3.5 points at 5%**. The residual misses are **low-entropy — confidently wrong** — pixels, which no amount of entropy-ranked review recovers: exactly why the calibration audit on the next slide matters.](images/mcdropout_operating_curve.png){fig-align="center" width="100%"}
:::
:::
...
::: {.columns}
::: {.column width="33%"}
![Seed design only (6 cubes): GP $\sigma$ (grey) high everywhere; the estimated $\rho_{\text{rel}}{=}0.995$ boundary (blue) is far from the true one (orange, dashed) — 20% misclassified.](images/al_lpbf_iter00.png){width="100%"}
:::
::: {.column width="33%"}
![After 10 picks (16 exp): AL routes every cube to the boundary — misclassification drops to 2.7%.](images/al_lpbf_iter10.png){width="100%"}
:::
::: {.column width="33%"}
![After 25 picks (31 exp): Both window boundaries are sharply localized; grid at this res = 196.](images/al_lpbf_iter25.png){width="100%"}
:::
:::
...
::: {.notes}
**What the images show:** Actual AL run on surface fit to the Diaz-Vallejo 316L dataset ($\sigma=0.002$ noise), not synthetic data. GP with straddle acquisition focuses sampling on the process window's *boundary* (where it's hardest and most valuable). 6 seeds → 16 → 31 experiments, and the process window is quickly localized; grid would need 196. Simulated campaign — mechanism, not the result, is key: AL clusters effort where the boundary is sharpest.

**Steep early gain:** UCB/EI/straddle focus cubes right where high uncertainty (variance) meets high value: the *edge* of the process window. The interior fills in last.
:::
...
- **Campaign results (8 seeds):** 31 AL cubes confidently identify 83% ± 5% of the true window; a $9 \times 9 = 81$-cube grid is needed to match.  
  ⇒ **~2.6× reduction in experimental cost.**
- **Budget impact:** At €300/experiment: **€15,000 saved per material**.  
...
::: {.notes}
**Measured, not quoted:** Real campaign, real numbers: 8 random seeds; average 31 AL cubes = 83% window found; grid needs 81. AL consistently gets more window for fewer experiments. At typical lab cost, this is €15k *per alloy*, and effects multiply in higher input dimensions.

**Framing for students:** Grid-search is still what most labs do by default (“let’s sweep a grid”). AL gives a path to accelerate discovery, save resources, and shrink timelines — *especially* as the parameter space grows. Plot headline numbers up front, then explain why AL’s focusing behavior makes this possible.
:::
...
::: {.fragment}
![Active learning (blue) vs $k{\times}k$ grid sweep + the same GP (orange), both on the Diaz-Vallejo-grounded surface. AL reaches 83% of the window at 31 cubes; the grid needs 81 to match. Both saturate near ~90%: the last sliver of window hugs the boundary, where $P > 0.9$ classification would require replicate cubes.](images/al_lpbf_effort_curve.png){width=40% fig-align="center"}
:::
