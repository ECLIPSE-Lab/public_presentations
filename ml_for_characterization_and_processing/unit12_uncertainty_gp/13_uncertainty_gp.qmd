...
### §A · From MFML theory to lab practice {style="text-align: center; margin-top: 15%;"}
...
::: {.column width="45%"}
::: {.fragment}
![A probability corridor around a process signal: the conditional distribution $p(x\,|\,t)$ — not a point estimate — is what a threshold decision reads off.](images/process_corridor.png){width="100%"}
:::
:::
:::
...
### §B · Picking a UQ method per lab task {style="text-align: center; margin-top: 15%;"}
...
### §C · Case study A — GP for process→property mapping {style="text-align: center; margin-top: 15%;"}
...
- Real published dataset: **5 temperatures × 10 specimens = 50 values**, published to the last indent (Table 3 of the paper; `data_mantzoukas2021_table3.csv`).
:::
:::
...
::: {.notes}
**Why this alloy, why this data set.** 21CrMoV5-7 (DIN 1.7709) is a workhorse Cr-Mo-V quench-and-temper steel for pressure-vessel and rotor applications — and the Mantzoukas et al. 2021 paper (MATEC Web Conf. 349, 02005, open access) publishes the *complete raw data*: 50 randomized specimens from one bar batch, five tempering groups, every HV30 value in Table 3. Everything on the next four slides is computed from that table — the CSV and the fitting script (`make_gp_figures.py`) are in the unit folder, so students can reproduce every number.

**Physics in one breath.** In plain-carbon steels tempering monotonically softens. *Not here*: in Cr-Mo-V grades the 475-575 °C window is exactly where **secondary hardening** happens — Cr/Mo/V alloy carbides precipitate and re-strengthen the matrix, competing with the recovery of the martensite. The measured response is nearly flat overall (379-409 HV30) with peaks at 500 and 550 °C. The paper's own conclusion: the grade "resists tempering back" in this window. This non-monotonic fine structure is the whole reason a parametric softening law fails and kernel choice (next slide) matters.

**Why the group means are trustworthy.** 10 replicates per condition, each specimen itself the mean of six HV30 indents, all specimens randomized from one batch (Minitab randomization, one furnace). Standard error of a group mean ≈ $\sigma_n/\sqrt{10} \approx 4$ HV30 — so the 500 °C peak (409 vs 381 at the neighbours) is a ~7-SE effect, unambiguously real signal, not noise.

**Why $n = 50$ is still "small data".** Fifty scalar measurements is far too small for any neural network to give honest uncertainty, and exactly the regime where a GP shines. Also note the heteroscedasticity in Table 3: group SDs run from 4.6 (575 °C) to 20.2 HV30 (500 °C) — precipitation-driven scatter. We fit a homoscedastic GP with the pooled SD and flag the heteroscedastic extension in the notes of slide 09.

**Anti-pattern.** Students will try to fit a polynomial regression with bootstrapped CIs. On this data a low-order polynomial *averages away* the two secondary-hardening peaks and a high-order one chases the noise. The GP is the *right* tool because it has a principled prior (correlation length) and a principled posterior CI.

**Cross-reference.** Mantzoukas_2021_17709 in `ref.bib`; full PDF in the unit folder. MFML W12 for the posterior derivation. ML-PC u02 (physics-of-data) for the discussion of replicate scatter vs model uncertainty.
...
::: {.column width="45%"}
::: {.fragment}
![GP fit to the actual Mantzoukas Table-3 data (all 50 specimens): the CI on the mean response narrows at the five sampled temperatures and widens between them; the light band adds the replicate noise.](images/gp_17709_fit.png){width="100%"}
:::
:::
:::
...
1. Small-data calibration: with $n = 50$ the GP's posterior CI is *honest by construction* — no need for a separate calibration set. Neural-net UQ at $n = 50$ is fantasy. And we *check* the construction: leave-one-specimen-out coverage on this fit is 46/50 = 92% for the nominal 95% band (slide 10).
2. Correlation-length prior: Matérn 5/2 encodes "the response varies smoothly with a characteristic temperature scale" — exactly the inductive bias precipitation kinetics gives you for free. Note it does *not* assume monotonicity, which is what saves us here.
3. Interpretable hyperparameters: $\ell$ is a *characteristic temperature scale* the metallurgist can read — here 13 °C, i.e. the secondary-hardening peaks are narrower than the 25 °C sampling grid. Try explaining a 100-parameter MLP's hidden activations to your lab manager.

**Walk the figure.** All 50 raw specimens (open circles), group means ± 95% CI (red diamonds), GP posterior mean, dark band = 95% CI on the mean response, light band = predictive interval including replicate noise. Point at the 449 HV30 specimen at 500 °C: a genuine outlier the paper kept; the predictive band absorbs it, the mean band does not chase it.
...
**Materials-specific pre-empt.** "Why not a physics-based parametric model (Hollomon-Jaffe parameter)?" — Hollomon-Jaffe assumes monotonic tempering-back and *cannot represent secondary hardening at all*; on this alloy it is structurally wrong in exactly this window. The defensible hybrid is Hollomon-Jaffe (or any parametric softening law) as the GP's prior mean with the GP correcting deviations — mention that the hybrid exists; here the non-parametric GP alone is the honest choice.

**Cross-reference.** MFML W12 for the marginal-likelihood objective and the Cholesky-based solver; ML-PC u11 (PINNs, the previous deck) for the physics-prior hybrid.
...
::: {.column width="40%"}
::: {.fragment}
![Samples from the GP prior with the hyperparameters fitted to the 1.7709 data (left) and with a deliberately oversmoothed length scale (right); red: the measured group means.](images/gp_17709_prior_samples.png){width="100%"}
:::
:::
:::
...
**Trap to flag explicitly: $\sigma_n^2$ as a model-misfit sponge.** If you let the optimiser pick all three hyperparameters freely, it will absorb model misfit (wrong kernel, wrong mean function) into $\sigma_n^2$, blowing up the noise estimate. The right discipline — used for this actual fit — is to estimate $\sigma_n^2$ from the replicates (pooled within-group SD of Table 3: $\sigma_n = 13.4$ HV30) and *fix* it during marginal-likelihood optimisation. This single discipline change is the most common audit-fix I make on student GP code.

**The second trap, which this data actually springs: degenerate $\ell$ under free MLE.** With 25 °C spacing and an up-down-up-down group-mean pattern, the marginal likelihood is nearly flat for all $\ell$ below the grid spacing and the optimiser runs $\ell \to 0$ (the "every condition is independent" limit). The fix is the one slide 11's caveat preaches: an informative prior. We used $\ell \sim \text{LogNormal}(\ln 25\,°\text{C}, 0.5)$ — centred on the sampling spacing — and took the MAP, landing at $\ell = 12.6$ °C. Lesson: the data *cannot resolve* structure finer than its own sampling grid, and the hyperparameters will tell you so if you listen.

**Heteroscedasticity footnote.** Table 3's group SDs range 4.6–20.2 HV30 (largest exactly at the 500 °C precipitation peak). We pool to a single $\sigma_n$ for simplicity; the honest extension is a heteroscedastic noise model with per-group $\sigma_n$ — good MSc-thesis exercise.

**Why Matérn 5/2 for metallurgy.** RBF is $C^\infty$ smooth — every derivative exists. Real tempering responses have sharp features (here: secondary-hardening peaks barely wider than 25 °C). Matérn 5/2 is twice differentiable but not more — it can bend more sharply where the physics calls for it. Bishop ch.6 has the full kernel zoo.
...
**Anti-pattern.** Reporting a GP fit without listing the hyperparameter values and how each was set. The reproducible statement for this fit is: "Matérn 5/2, $\ell = 13$ °C (MAP, LogNormal prior at the 25 °C spacing), $\sigma_f = 13$ HV30 (MLE), $\sigma_n = 13.4$ HV30 (fixed from replicates)". If a paper does not give you the equivalent sentence, treat its GP as unreproducible.
...
::: {.column width="45%"}
::: {.fragment}
![The real test: the 525 °C group (red) was hidden from the GP, which was fit on the other 40 specimens. The held-out group mean lands inside the 95% band.](images/gp_17709_heldout.png){width="100%"}
:::
:::
:::
...
::: {.notes}
**The shape of the posterior matters more than any single number.** Walk the class through the ribbon: tight at sampled points (with 10 replicates the CI on the mean collapses to $\approx \pm 2\sigma_n/\sqrt{10} \approx \pm 8$ HV30), wide mid-gap ($\pm 19$ HV30 — the prior takes over), reverting toward $\sigma_f$ outside the data (next slide). That ribbon shape is the whole reason we used a GP.

**Leave-one-out CV is the GP's free diagnostic.** Because the GP posterior is closed-form, leave-one-out predictions are *cheap* (Sundararajan-Keerthi trick: no refit per fold; our script brute-force refits because $n = 50$ costs milliseconds). On this fit: 46/50 specimens inside the 95% predictive band — 92% empirical vs 95% nominal, i.e. the band is honest to within the resolution 50 points can check ($\pm$ a few points is expected binomial fluctuation).

**Why the held-out-group check is the one to show.** Held-out *specimens* mostly test the noise model; a held-out *condition* tests interpolation — the thing we actually want the GP for. The GP fit without the 525 °C group predicts $399 \pm 26$ HV30; measured $382 \pm 5$. Two readings: (a) covered — the CI is honest; (b) the point prediction pulled up toward the neighbouring peaks and missed by 17 HV30, and the wide band *told you not to trust the point estimate*. Both readings are the lesson. If your process decision needs $\pm 10$ HV30 at an unsampled temper, the GP's answer is "measure it" — and it says so quantitatively.
...
**Cross-reference.** §6 reliability diagrams.
...
::: {.column width="45%"}
::: {.fragment}
![The actual fit, extended beyond the data: inside 475–575 °C the band tracks the measurements; outside, it reverts to the prior within a couple of length scales — the honest answer.](images/gp_17709_extrapolation.png){width="90%"}
:::
:::
:::
...
**Materials hook.** Note what the GP reverts *to*: the prior mean, here the grand data average (391 HV30). Below 475 °C that is physically wrong — the response climbs toward the as-quenched $42 \pm 2$ HRC (the paper reports 40–44 HRC after quench, i.e. 392–434 HV) as carbide precipitation slows. The GP does not know this; the wide band is it admitting so. If you *have* that physics intuition, encode it: a parametric tempering-law mean function (with the GP correcting deviations) would extrapolate downward sensibly *and* keep honest variance growth. With $\ell = 13$ °C the reversion happens within ~2 length scales — 25-30 °C past the last data point the model is already back at the prior. Short length scale = short memory; that is the price of resolving the peaks.
...
**Pre-empt.** "Could I use a periodic kernel? The fitted curve looks oscillatory." — good eye, and no: the two peaks come from *distinct* precipitation reactions (Cr-, then Mo/V-rich carbides), not from a periodic mechanism. A periodic kernel would hallucinate more peaks outside the data with high confidence — the exact opposite of honest extrapolation. Periodic kernels belong where physics is genuinely periodic (cyclic loading, rotating-disc spectra).
...
**Forward link.** §6 audits calibration with reliability diagrams; TabPFN's calibrated posterior already gives close-to-honest intervals on in-distribution data — still verify on a held-out lab set.
:::


## {background-color="#1a1a2e"}

### §D · Case study B — MC Dropout for SEM defect segmentation {style="text-align: center; margin-top: 15%;"}
...
::: {.column width="55%"}
::: {.fragment}
- Task: per-pixel segmentation of SEM micrographs of an additively manufactured steel into {matrix, austenite, martensite/austenite, precipitate, **defect**} — the public **MetalDAM** set [@metaldam; @luengo_2022_metaldam].
...
**The data is real; the tool shift is simulated.** All §D figures come from an actual run (`make_mcdropout_figures.py`, seed 0) on MetalDAM — 42 SEM micrographs of AM steel from ArcelorMittal with expert pixel labels; class shares are heavily imbalanced (austenite 58%, matrix 32%, M/A 9%, defect 0.7%, precipitate 0.2%). MetalDAM comes from a single instrument, so the "SEM #2" evaluation applies a physically motivated detector perturbation (gamma 0.75, contrast ×1.3, brightness +0.04, extra noise σ=0.02) to the held-out micrographs — say this out loud; on your own data you should always *evaluate on a tool you did not train on*. (For a multi-category SEM benchmark without pixel masks, see Modarres et al. [@Modarres_2017_SEM] — classification only, which is why it is not used here.)
...
**Compute budget (measured).** $T=30$ on a 256×256 tile took 0.07 s on an RTX A6000 in our run (`make_mcdropout_figures.py`); on a 1080Ti-class workstation card expect roughly 1–2 s per tile. Fine for offline analysis (tens of tiles/day on a benchtop SEM), *too slow* for inline detection at 1 Hz on modest hardware. If real-time, drop to ensembles (parallelisable) or temperature-scaled deterministic + Mahalanobis OOD.
...
![Two held-out MetalDAM micrographs: SEM input, MC-mean prediction, per-pixel predictive entropy ($T{=}30$). **Low entropy** inside well-formed austenite/matrix regions; **high entropy** at phase boundaries, inside ambiguous martensite/austenite laths, and at defect rims.](images/mcdropout_maps.png){height=700px fig-align="center"}
...
**What the figure actually shows (our MetalDAM run).** High entropy concentrates (i) on austenite/matrix phase boundaries, (ii) *inside* martensite/austenite regions — the class the annotators themselves find hardest, IoU 0.43 — and (iii) at defect rims. Precipitates are essentially unlearnable in MetalDAM (IoU ~0.01) because most were left unannotated (stated in the dataset README) — a live example of the u03 label-noise lecture.

**The generic failure modes to name beyond this figure.**
...
- Charging zones: physical SEM artefact; the model is *correctly refusing* — pass it to a human or to a charge-compensation preprocessor. (MetalDAM has no charging halos; show one from your own instrument if available.)
...
![Measured on the held-out MetalDAM micrographs: fully automatic recovers **82.9%** of defect pixels; entropy-ranked review adds **+1.4 points at 1%** review and **+3.5 points at 5%**. The residual misses are **low-entropy — confidently wrong** — pixels, which no amount of entropy-ranked review recovers: exactly why the calibration audit on the next slide matters.](images/mcdropout_operating_curve.png){width="52%" fig-align="center"}
...
**The operating-curve mindset.** Treat this like an ROC curve: the model gives you a sliding tradeoff, and the deployment context picks the operating point. "86 % recall at 5 % review" is *one point on the curve*; the *curve itself* is the deliverable. Always plot it.

**Numerical interpretation (our run).** Auto-only recall is already 82.9% because the weighted loss makes the model defect-sensitive; entropy-ranked review then buys +1.4 points at 1% review and +3.5 at 5% (89.2% at an unrealistic 20%). There is no sharp knee here, and the gap to 100% is the honest headline: the remaining defect pixels are *confidently wrong* — the entropy map cannot flag them, only recalibration (next slide) or more labels can. At 5% review on a 1024×703 micrograph that is ~36k flagged pixels — clusterable into a few hundred ROIs an analyst scans in under a minute.
...
::: {.notes}
**The single most underrated slide in the deck.** Most students who deploy a model and watch performance drop will immediately assume *retraining* is the fix. Wrong first move: run the calibration audit ($\sim 50$ images, 5 minutes of work) before touching the weights. Be precise about what recalibration buys — our run shows both sides: temperature scaling restored *honest confidences* (ECE 9.2% → 3.5%), so every confidence-thresholded downstream step (reject-for-review, defect triage) works again, but it cannot restore raw pixel accuracy (argmax is unchanged; 59% stays 59%). If the accuracy loss itself is unacceptable, that is a fine-tuning decision — made *after* the audit, not instead of it.
...
**War story.** A collaborator's pore-detection *pipeline* (segmentation + confidence-thresholded defect triage) dropped from 96 % to 78 % end-to-end after a routine chamber clean. They retrained for two weeks and recovered to 91 %. The actual bug: the contrast shift had broken the *calibration*, so the fixed confidence threshold silently discarded most true detections — temperature scaling on 30 calibration images restored the thresholding step in an hour. (Note the mechanism: recalibration can rescue a confidence-*thresholded pipeline*; it never changes the raw argmax accuracy.) The training time was *wasted*; the diagnosis was the bug.
...
### §E · Case study C — Active learning loop for AM process windows {style="text-align: center; margin-top: 15%;"}
...
- Two-axis design space: laser power $P \in [125, 350]$ W and scan speed $v \in [100, 3400]$ mm/s — the ranges of the 35-cube dataset of @DiazVallejo_2021_316L that grounds this case study.
...
## 22. Result: pareto frontier of effort vs window discovered
...
::: {.fragment}
- **Measured on this campaign** (8 random seeds, next slide): **31 AL cubes confidently identify 83% ± 5% of the true window; a 9×9 = 81-cube grid is needed to match** — a **~2.6× reduction in experimental cost**.
- At €300/experiment: **€15 000 saved per material** — and the AL advantage *grows* with input dimension (hatch, layer, preheat...) [@Hernandez-Lobato_2014_PES].
:::

::: {.fragment}
::: {.columns}
::: {.column width="33%"}
![Seed design only (6 cubes): GP $\sigma$ (grey) high everywhere; the estimated $\rho_{\text{rel}}{=}0.995$ boundary (blue) is far from the true one (orange, dashed) — 20% of the plane misclassified.](images/al_lpbf_iter00.png){width="100%"}
:::
::: {.column width="33%"}
![After 10 AL picks (16 experiments): the straddle acquisition routes every cube to the window boundary — misclassified area down to 2.7%.](images/al_lpbf_iter10.png){width="100%"}
:::
::: {.column width="33%"}
![After 25 AL picks (31 experiments): both window boundaries (keyhole below, lack-of-fusion above) are localized; a grid at this resolution needs 196.](images/al_lpbf_iter25.png){width="100%"}
:::
:::

AL campaign against a ground-truth density surface fit to the 35-cube 316L dataset of @DiazVallejo_2021_316L; measurement noise $\sigma = 0.002$ = the dataset's median replicate scatter. Generated by `make_gp_figures.py` in the unit folder.
:::

::: {.callout-note .fragment}
Active learning beats the lab's *actual* default — the grid sweep — not just random sampling.
:::

::: {.notes}
**About the three panels.** They are an *actual AL run* from `make_gp_figures.py` (in the unit folder), grounded in real data: the ground-truth $\rho_{\text{rel}}(P, v)$ surface is a GP fit to the 35 316L cubes of Diaz Vallejo et al. 2021 (Metals 11:832, CC-BY; layer 30 µm, hatch 120 µm; heteroscedastic noise from the reported replicate std), and the campaign queries it with $\sigma = 0.002$ noise — the dataset's own median replicate scatter. 6 seed cubes, then a GP (anisotropic Matérn 5/2) with the *straddle* acquisition $1.96\sigma - |\mu - 0.995|$ that targets the level set rather than the maximum. Say openly that the *campaign* is simulated (the cubes were printed by Diaz Vallejo, not picked by our loop) — the point of the panels is the *mechanism* (picks cluster on the boundary; $\sigma$ collapses there first). Window misclassification area drops 20% → 2.7% → 3.0% across the panels while a 14×14 grid at the same resolution costs 196 cubes. The real window shape is visible in the panels: keyhole boundary near $v \approx 400$ mm/s, lack-of-fusion boundary climbing from $\approx 1300$ to $\approx 2100$ mm/s as power rises.

**The headline numbers — measured, not quoted.** On this campaign (8 seeds, next slide): 31 AL cubes confidently identify 82.6% ± 4.9% of the true window; the grid needs 9×9 = 81 cubes to match. That is ~2.6× — real but smaller than the 6-7× often quoted, because in 2-D a grid is a respectable design. The AL advantage grows with input dimension (add hatch spacing, layer thickness, preheat and the grid explodes combinatorially while AL does not). At €300/experiment the 2-D saving is already €15k per alloy/system; across a lab's portfolio of 5-10 alloys/year that is €75-150k *and* the time-to-window-map drops from months to weeks.
...
**The grid-search baseline is the comparison that matters.** Grid-search is what *every* materials lab does by default ("we'll do a 14×14 sweep"). The slide is *not* "AL beats random sampling" (a weak claim); it is "AL beats the actual current practice ~2.6× in 2-D, more in higher dimensions". That framing is what gets the slide photographed.

**Anti-pattern.** Reporting the AL curve without an error band. The pareto curve has Monte-Carlo variability (different random initialisations give different curves). Always plot the *mean curve over 5-10 seeds* with a shaded band — the next slide does exactly this. Single-seed AL curves are not publishable evidence.
...
::: {.fragment}
![Active learning (blue) vs $k{\times}k$ grid sweep + the same GP (orange), both on the Diaz-Vallejo-grounded surface. AL reaches 83% of the window at 31 cubes; the grid needs 81 to match. Both saturate near ~90%: the last sliver of window hugs the boundary, where $P > 0.9$ classification would require replicate cubes.](images/al_lpbf_effort_curve.png){width=70% fig-align="center"}
:::
...
::: {.notes}
**How to read the figure.** Same ground truth, same GP, same noise — the *only* difference between the curves is which cube gets printed next. The straddle acquisition spends every cube on the level set; the grid spends most cubes in the interior and the corners where the classification is already easy or hopeless.

**The honest numbers.** AL at 31 cubes: 82.6% ± 4.9% of the true window confidently identified. The grid needs 9×9 = 81 cubes to reach that level. That is a ~2.6× cost reduction in this 2-D, 316L-grounded campaign — real, but *not* the 6-7× sometimes quoted. Two reasons: (a) in 2-D a grid is a respectable design; the AL advantage grows with input dimension (add hatch spacing, layer thickness, preheat...); (b) both curves saturate near 90% because the remaining window area hugs the boundary, where confident classification at σ = 0.002 needs replicates, not more single cubes.

**Why the bands matter.** This is the anti-pattern from the previous slide done right: 8 campaigns with jittered seed designs and fresh noise. The AL band (± 5%) is the honest statement of run-to-run variability; a single lucky seed can look 10 points better.

**Pre-empt.** "Why does the grid curve wobble downward at some sizes?" — coarse grids can straddle the boundary luckily or unluckily; the GP's confident region shifts with where the grid points happen to land relative to the level set.

**Pacing.** ~2 minutes. Read the two numbers off the plot, state the two saturation reasons, move on.
:::
...
### §F · Calibration & deployment hygiene {style="text-align: center; margin-top: 15%;"}
...
### §G · Wrap {style="text-align: center; margin-top: 15%;"}
...
5. **The math lives in MFML W12 (Bayes, GP, calibration).** ML-PC W13 is about **using** that math to save tests, find process windows, and ship trustworthy models.
:::
...
::: {.notes}
**The five-line takeaway.** Each of the five items in the recap maps onto a specific section of the deck:
...
3. UQ enables AL → §5 (AM process windows); UQ is the *engine*, not the ornament.
4. OOD detection → §6 Mahalanobis; calibration and OOD are *separate* responsibilities.
5. The math is in MFML W12 → bridge to the sibling course.
...
**Exam hint without saying "exam".** If the exam contains an open-ended question about deployment, the rubric is this slide. Tool choice + calibration + AL + OOD + math pointer = full marks. Tell them.

**What the homework will ask.** Take the §3 GP example, run it on your own tabular data set, produce a leave-one-out reliability diagram, write the model-card uncertainty paragraph. That homework is the practical test of whether the unit landed.

**Back link.** Unit 11 (PINNs, the *previous* deck) showed how to bring physics into the model itself — replacing parts of the data-driven prior with a hard physics prior. UQ + PINNs together is the modern frontier.
...
- &larr; Previous: [Unit 11 &mdash; Physics-informed and constrained ML](../unit11_pinns/12_pinns.html)
- &rarr; Next: [Unit 13 &mdash; Integration, limits, and reflection](../unit13_reflection/14_reflection.html)
...
- **Mantzoukas et al. (2021)** — 21CrMoV5-7 quench-and-temper data used in §C [@Mantzoukas_2021_17709].
- **Modarres et al. (2017)** — SEM benchmark for the segmentation/classification setup of §D [@Modarres_2017_SEM].
- **Diaz Vallejo et al. (2021)** — 316L L-PBF process-parameter/density dataset grounding the §E active-learning campaign [@DiazVallejo_2021_316L].
