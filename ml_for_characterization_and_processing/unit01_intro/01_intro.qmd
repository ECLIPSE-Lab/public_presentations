...
::: {.notes}
Pause after the question. Let students discuss in pairs for ~1 minute.

Prompt them:
- What limits physics-only approaches?
- What fails with pure ML at n=50?

Then reveal the answer and walk through the decomposition idea.
:::
...
::: {.notes}
This is a key pedagogical moment — connecting the abstract categories to a concrete decision.
Emphasize:
- Black-box isn't rejected just because it's ML, but because high capacity models overfit n=50.
- White-box isn't rejected because it can't handle composition, but because it misses key state variables (grain size, etc).
- Grey-box formulation maps exactly to how modern materials informatics handles priors.
:::
...
::: {.notes}
**Teaching Guide for this slide:**

**1. Context (The "What"):**
- Briefly clarify the physical process: Quenching hardens steel but leaves it brittle; tempering (reheating) generally softens it to recover toughness. 
- **CRITICAL DETAIL**: The plot does NOT show a simple linear drop. This steel (21CrMoV5-7) exhibits a **secondary hardening peak** around 500-600°C due to fine alloy carbide precipitation. This makes the landscape highly **non-linear**, explicitly requiring models that can capture complex, oscillating physics rather than a simple straight line!
- From an ML perspective, this is classical **tabular regression**. The targets are continuous numbers (Hardness) and the inputs are tabulated process parameters.

**2. The Crux (The "Why it's hard"):**
- Focus heavily on the **"Risks"** bullet. This is the main pedagogical takeaway.
- A spreadsheet might simply log: `Quench=Oil`, `Temp=600°C`. But a 1-ton industrial forging and a 10-gram lab coupon quenched in the *exact same oil* will cool at vastly different rates due to thermal mass. 
- Furthermore, placement inside the furnace matters (outside parts get hotter than center parts).

**3. The Takeaway:**
- What looks like simple regression is riddled with "hidden state variables" unrecorded in the dataset. 
- If an ML model succeeds on lab data but fails in a factory setting, the algorithm isn't broken—the dataset just failed to capture the true physical state of the process (section size, furnace kinetics). This teaches students to aggressively question their CSV files.
:::
...
**1. Context (The "What"):**
- TiO2 has distinct crystal phases (Anatase, Rutile), which produce distinct Raman scattering peaks. 
- The ML Task: **1D Signal Classification**. Instead of a spreadsheet or a 2D image, the input is an array of intensities over a frequency axis (Wavenumber cm⁻¹).
- Why use ML instead of standard peak fitting? ML can analyze noisy, low-exposure spectra incredibly fast, enabling rapid spatial mapping of variations across large samples.

**2. The Crux (The "Why it's hard" - Focus on Risks):**
- **Baseline Drift:** Real samples fluoresce, causing the entire spectrum to tilt or slope upward. If untreated, the ML model might lazily classify samples based on the shape of their *background fluorescence* rather than the actual atomic Raman peaks (a classic "shortcut").
- **Calibration Instability:** The ML model learns peak locations at specific array indices. If the spectrometer laser or grating shifts by just a few cm⁻¹ between months, the peaks move. An overfitted ML model will fail completely because the features "shifted columns".
- **Preprocessing Leakage:** Scaling and normalizing spectra is mandatory, but if you calculate the global mean/variance across your *entire* dataset before splitting it, information from your test set "leaks" into the training set, causing deceptively high metrics.

**3. The Takeaway:**
- When working with spectra, **physics-based preprocessing** (background subtraction, peak alignment, standard normal variate scaling) is usually far more critical than picking the newest ML algorithm.
:::
...
::: {.notes}
CRISP-DM is an industry-standard template. As noted in the course text, we emphasize a 7-phase cycle that explicitly treats "Monitoring" as a separate phase. Here we walk through a unified continuous example: "Predicting alloy yield strength to reduce destructive testing costs."
:::
...
::: {.notes}
This is the most overlooked phase. Students jump to "let me train a model" without defining success. A clear question is worth more than a clever algorithm. If the cost of modeling exceeds the savings of 50 fewer tests, the project fails here!
:::
...
::: {.notes}
Exploratory Data Analysis (EDA) is essential. Show histograms, scatter plots. As the textbook emphasizes, data without units is worthless. Missing values or unrecorded background conditions invalidate projects later. Find them now.
:::
...
::: {.notes}
Emphasize the train/test split. Random splitting is the #1 mistake in materials ML — if samples from the same physical batch end up in both the train and test sets, the model overfits to the *batch history* rather than learning the physics!
:::
...
::: {.notes}
If simple linear regression achieves an R² of 0.85, you need a very compelling reason to use a black-box model. The marginal improvement rarely justifies the loss of explainability (which the textbook emphasizes is vital for building engineering trust).
:::
...
::: {.notes}
Generalization is the ultimate goal. The real "gold standard" test in materials science: does the model still work if we give it data generated by a *different lab* or *different operator*? 
:::
...
::: {.notes}
As the textbook points out, Monitoring (Phase 7) is tracked explicitly in real environments because physical processes experience drift. Sensor recalibration or new raw material suppliers will shift the underlying relationships over time!
:::
