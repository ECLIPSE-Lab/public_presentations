---
title: 'Machine Learning in Materials Processing & Characterization<br>Unit 8: Probabilistic Sequence Modelling for Materials Processes'
author:
...
## 02. Where we are this week

::: {.columns}
::: {.column width="50%"}
**Recap — Unit 7 (last week).**

- Generalisation, robustness, process windows.
- Validation strategies that survive distribution shift.
- "What does the model *not* know" entered the room.
:::
...
::: {.notes}
**Curriculum-week note (read for yourself, do not say out loud).** Folder still named `unit07_time_series/` because we deferred the rename to term-end. *On the lecture day this is W8.* If a student asks why the slide says Unit 8 and the URL says unit07, point them at `REALIGNMENT_2026-05-09.md` and move on.
...
**Where RNNs win.** Online inference where you must produce $\hat x_{t+1}$ as soon as $x_t$ arrives, without re-running a windowed convolution; long, irregular sequences; tasks where the *state* of the process matters (think Kalman filter — Slide 36).
...
3. *No anomaly detection that scales.* The "anomaly = low likelihood" recipe (Slide 37) needs a likelihood.
...
- Low-noise sensors.
- Educational examples (sine recovery — Slide 42).
:::
...
**Forward link to Slide 31.** The "dropout between layers but not within" rule has an exception: MC dropout (used at inference for epistemic uncertainty) needs *recurrent* dropout that is consistent across time. This is a non-trivial implementation detail; most published MC-dropout-LSTM papers get it wrong. We will see this on Slide 31.
:::
...
**MFML W8 callback.** Everything in this section assumes MFML W8 vocabulary: aleatoric, epistemic, MLE for Gaussian likelihood, MDN, calibration plot, KL divergence. If the audience is rusty, do a 60-second whiteboard recap of the Gaussian NLL — the formula on Slide 28 is otherwise opaque.

**Shape of the section.**

1. *Why two kinds of uncertainty?* (slides 24–26)
2. *How do I get a predictive distribution?* (heteroscedastic Gaussian, MDN — slides 27–30)
3. *How do I get epistemic uncertainty?* (MC dropout, ensembles — slides 31–32)
4. *Is my predictive distribution any good?* (calibration, recalibration, sharpness — slides 33–35)
5. *What about the classical view?* (Kalman — slide 36)
6. *Application: anomaly detection* (slide 37)
:::

## 24. Aleatoric vs epistemic in sensor streams
...
**MFML W8 callback.** This is the same picture they saw two days ago in MFML W8 (and in MFML's W3 "Aleatoric vs Epistemic" slide and Unit 2 of THIS course Slide 24). Do not re-derive — confirm the recall.
...
## 25. Worked example — separating the two
...
1. Train $K$ models with different seeds (or use MC dropout — Slide 31).
...
## 26. From point prediction to predictive distribution
...
- **Heteroscedastic Gaussian** (Slide 27): $\mathcal{N}(\mu_t, \sigma_t^2)$.
- **Mixture density** (Slides 29–30): $\sum_k \pi_{k,t}\,\mathcal{N}(\mu_{k,t}, \sigma_{k,t}^2)$.
...
## 27. Heteroscedastic Gaussian head
...
- Add MC dropout / ensembles (Slides 31–32) for epistemic.
:::
:::
...
**Calibration is not yet guaranteed.** Even with a perfectly trained heteroscedastic Gaussian head, the predictive *distribution* is only as well-calibrated as the model is correct. We will measure it explicitly on Slides 33–34.
:::

## 29. Mixture density network (MDN) head — architecture
...
## 30. MDN — training, inference, and mode collapse
...
**For the case studies.** When we get to melt-pool monitoring (Slide 39), the MDN will be the head that makes the difference. Tease this now.
:::

## 31. MC dropout for epistemic uncertainty
...
## 32. Deep ensembles for epistemic uncertainty
...
## 34. Recalibration — Platt and isotonic
...
**Beyond Platt and isotonic.** Quantile regression, conformal prediction (which also gives finite-sample guarantees), and Beta calibration are all in scope of UQ research. We mention only the two classical ones; W12 (Uncertainty + GPs) develops more.
:::

## 35. Sharpness vs calibration trade-off
...
## 36. State-space view — Kalman as the linear-Gaussian limit
...
## 37. Anomaly detection via predictive likelihood
...
**Calibration matters here too.** A predictive-likelihood anomaly detector is only as good as its calibration. If the model is over-confident, *every* observation looks anomalous — false-alarm rate is unusable. This is the tightest practical reason to spend time on calibration plots (Slides 33–34).
...
## 38. Melt-pool monitoring (1/2) — deterministic baseline
...
## 39. Melt-pool monitoring (2/2) — MDN-LSTM with uncertainty
...
## 40. RUL prediction (1/2) — point vs interval
...
## 41. RUL prediction (2/2) — survival-curve view
...
## 42. Recovering frequency from a noisy sine — McClarren example
...
## 44. Real-time feedback loops with risk thresholds
...
**The other two sentences** (preview of Slide 49):
...
## 47. Feature scaling and leakage
...
## 48. Evaluation — horizon and proper scoring
...
::: {.fragment}
**The mantra (Slide 35 reprise).** Use proper scoring rules. MSE on the mean is *not* one.
:::
...
## 49. Summary — three sentences to take home
...
::: {.fragment}
2. **Probabilistic heads (Gaussian, MDN) + ensembles / MC dropout deliver predictive distributions.** Aleatoric is irreducible; epistemic is reducible.
:::

::: {.fragment}
3. **Calibration is a separate axis from accuracy.** Measure it, recalibrate if needed, and *then* tune thresholds. The Kalman filter is the linear-Gaussian classical limit.
:::
...
- &larr; Previous: [Unit 06 &mdash; Data scarcity & transfer learning](../unit06_transfer_learning/01_intro.html)
- &rarr; Next: [Unit 08 &mdash; Generalization, robustness, and process windows](../unit08_generalization_robustness/01_intro.html)
- [All courses](../../index.html)

<!-- END prev-next -->

## 50. References, forward look, exercise hand-off
...
- W10: attention / transformers — long-sequence successor.
- W11: automation — closed-loop control with the heads we built today.
- W12: GPs — another route to predictive distributions.
:::
...
**Office-hours preview.** The most common question this exercise generates is "why is my MC-dropout LSTM still over-confident?" Answer: PyTorch's built-in LSTM dropout does not implement variational dropout correctly (Slide 31 note); use a custom cell. This is the trap the bonus question reveals.

**For the next lecture (W9 — Inverse problems and process maps).** We move from "predict the next step given the past" to "infer process parameters given an observation". Same probabilistic vocabulary; very different inference machinery (variational inference, simulation-based inference). Today's MFML W8 / Unit 8 toolkit is the foundation.
