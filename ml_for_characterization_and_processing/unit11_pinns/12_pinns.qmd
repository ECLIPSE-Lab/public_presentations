---
title: 'Machine Learning for Characterization and Processing<br>Unit 11: Physics-constrained ML patterns for the lab'
...
### §A · From MFML theory to lab patterns {style="text-align: center; margin-top: 15%;"}

## 01. The MFML W13 toolkit (we will not re-derive these)
...
## 03. What physics to enforce in materials labs
...
### §B · Pattern A — Soft-constrained CNN for diffraction inversion {style="text-align: center; margin-top: 15%;"}

## 04. Setup: phase-fraction inference from XRD
...
## 05. The constraint — softmax is not enough
...
## 09. Result reporting — two axes, not one
...
### §C · Pattern B — PINN for AM melt-pool thermal {style="text-align: center; margin-top: 15%;"}

## 10. Setup: laser powder-bed fusion
...
::: {.column width="45%"}
::: {.fragment}
![PINN data flow: the network output feeds a data loss directly and, via automatic differentiation, a PDE-residual loss](images/pinn_arch.png){width=90%}
:::
:::
:::
...
## 13. Inverse problem: estimate the absorptivity
...
## 14. What goes wrong: gradient pathology
...
### §D · Beyond PINN — Neural-operator surrogates {style="text-align: center; margin-top: 15%;"}

## 16. Beyond PINN — Physics-Informed Neural Operators (PINO)
...
- For a **family** of PDE problems (parametric BCs, swept geometries, varying source terms), the right object to learn is the *solution operator*
$$
\mathcal{G}_\theta : \text{inputs} \;\longmapsto\; \text{solution field}.
$$
...
- **PINO** [@li_2024_pino] = FNO + the PINN residual loss. The operator both *fits* training solution pairs (data loss) and *satisfies* the PDE residual at sampled collocation points (physics loss).
:::
:::
...
::: {.fragment}
![A plain network on $x(t)$ (a) vs. a network fed both $x(t)$ and its Fourier transform $\mathcal{F}[x(t)]$ (b) — the Fourier-input idea behind FNO layers](images/fft_pinn.png){width=90%}
:::
:::
:::

::: {.notes}
30-second derivation: an FNO layer replaces the kernel integral $\int K(x,y)\,u(y)\,dy$ with a learned multiplication in Fourier space — FFT, point-wise complex weight, inverse FFT. The PINN residual is then computed on the *output field* of the operator. Why this composes cleanly: the operator is differentiable w.r.t. its inputs, so $\partial_t u$ and $\nabla^2 u$ at collocation points are just autodiff outputs of the operator forward pass. Anti-pattern: applying PINO to geometries far outside the training set — like any operator method, it extrapolates poorly; the PDE residual buys some regularization but does not turn a finite operator class into a universal solver. MFML W13 cross-reference: the PINO slide there derives the Fourier-layer math; here we just consume the result.
:::

## 17. GNN PDE Solvers — MeshGraphNets for Irregular Materials Meshes
...
- **MeshGraphNets** (Pfaff et al. 2021) [@pfaff_2021_meshgraphnets] treat the mesh as a graph: nodes carry physical state (displacement, temperature, stress), edges carry connectivity, and a GNN learns local **message-passing** updates that approximate one PDE time-step.
...
### §E · Pattern C — Symmetry-aware NN for the elastic tensor {style="text-align: center; margin-top: 15%;"}

## 18. Setup: predict Cᵢⱼ from a structure descriptor
...
## 19. Two paths to enforce symmetry
...
### §F · Pattern D — Physics-regularized time series for process drift {style="text-align: center; margin-top: 15%;"}

## 22. Setup: in-situ stress–strain during forming
...
## 24. How to enforce monotonicity
...
## 25. Sensor dropout — physics-aware augmentation
...
### §G · What still goes wrong {style="text-align: center; margin-top: 15%;"}

## 26. Constraint conflicts
...
## 28. When constraints become crutches
...
## 29. Pointer to MFML W13 (taught Mon 13.07 — after this unit)
...
### §H · Wrap {style="text-align: center; margin-top: 15%;"}
...
- &larr; Previous: [Unit 10 &mdash; Transformers for materials characterization (ViT, Flash Attention, Mamba)](../unit10_transformers_for_materials/transformers_for_materials.html)
- &rarr; Next: [Unit 12 &mdash; Uncertainty-aware regression & Gaussian Processes](../unit12_uncertainty_gp/13_uncertainty_gp.html)
