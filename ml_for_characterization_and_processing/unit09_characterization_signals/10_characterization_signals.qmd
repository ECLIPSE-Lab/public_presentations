...
## 11. Denoising via Reconstruction
...
## 16. AE Architecture: The Hourglass
...
## 18. Non-linearity: The Power of AE vs PCA
...
## 19. Training: Reconstruction Loss
...
## 24. Variational Autoencoders (VAE)
...
## 25. PCA vs. Autoencoder: Head-to-Head
...
::: {.column width="50%"}
::: {.fragment}
**The masking trick.** Masked Autoencoder [@he_2022_mae] adapted to 1-D spectra.
...
::: {.fragment}
**Loss.** MSE on **masked bins only**:
$$\mathcal{L}_{\text{MAE}} = \frac{1}{|\mathcal{M}|}\sum_{j \in \mathcal{M}} (x_j - \hat{x}_j)^2$$
The visible bins do not contribute — the model is forced to reconstruct what it has not seen.
:::
...
::: {.notes}
**Why this slide exists.** Sections 2 and 3 covered PCA and AE — both **purely unsupervised compression**. Neither one uses the unlabelled bulk to *improve* a downstream supervised task. MAE is the bridge: it is self-supervised pretraining, not just compression. The encoder learned on unlabelled spectra becomes the initialisation for the labelled task.

**One-sentence definition.** Masked Autoencoder is an autoencoder where the **input is corrupted by hiding 75% of the signal**, and the loss is computed only on the hidden part — forcing the encoder to learn structure that *generalises across the spectrum*, not just memorises local intensity.

**Why mask 75% and not 50%.** Empirical finding from the original vision paper, confirmed for 1-D spectra in subsequent work. At 50%, the network can interpolate masked bins from immediate neighbours — the task is too easy and the encoder learns a low-level smoothing function. At 75%, only a quarter of the spectrum is visible; the encoder must learn **what a spectrum of this class looks like** to fill in the gaps. Higher (90%) starts to underfit. Sweet spot is 75% for spectra with smooth + peak structure.

**Connection to the Unit-5 vanilla autoencoder.** Same MSE loss structure; same encoder–decoder architecture. The **only architectural change** is the input-side masking + mask tokens on the decoder side. The reason the result is dramatically better is that the loss is *self-supervised on hidden bins*, not *self-supervised on the full input* — the latter rewards trivial copying through the bottleneck, the former rewards modelling spectral structure.

**1080Ti budget.** A 4-layer, 128-dim transformer with 64 patches per spectrum has $\sim 1$M parameters. On 200 k spectra, batch 256, AdamW, $\sim 100$ epochs → trains **overnight on a single 1080Ti** (or in $\sim 2$ h on an A100). The fine-tuning step on the 100 labelled spectra takes minutes.

**Materials hook.** Every spectroscopy lab has years of archived data that was never labelled because labelling is the expensive step (an expert annotates each spectrum's phase / peak / valence by hand). MAE finally gives a reason to pay the storage cost: those archives become the pretraining corpus for *every future supervised task* the lab will run.

**Anti-pattern.** Applying MAE directly to a **labelled-but-scarce** dataset (100 spectra, no unlabelled bulk). The whole value proposition is *leveraging unlabelled data*. With only 100 spectra total, MAE gives you nothing — the encoder cannot learn spectral structure from 100 examples. In that regime, a simple CNN with strong augmentation or a Gaussian-process classifier beats MAE. **Rule of thumb: you need $\geq 10\times$ more unlabelled than labelled spectra before MAE pretraining pays off.**

**MFML W9 cross-reference.** The SSL refresh slide (MAE / DINOv2 / I-JEPA) introduced this family abstractly — masking-based self-supervision, contrastive self-supervision, joint-embedding predictive architectures. Today's slide is the **spectra-specific deployment** of the MAE branch. DINOv2 / I-JEPA would also work on spectra and are arguably better for very small downstream sets; we pick MAE here because reconstruction loss is the natural progression from PCA → AE → MAE in this unit's story.

**Chalkboard suggestion.** Draw a 64-bin toy spectrum on the board. Hatch out 48 of the bins (75%). Ask the class: "What information would you need to reconstruct the hatched parts?" — guide them to: peak positions, peak shapes, baseline. That **is** the spectral structure the encoder is forced to learn.

**Forward link.** The MAE encoder reappears in Unit 10 (Automation) as the on-the-fly feature extractor that an active-learning loop queries to decide which spectrum to label next.
:::
...
## 32. Multi-Spectral and Hyperspectral Data
...
## 33. Spatial-Spectral Autoencoders
...
## 34. Learning the Peak Dictionary: End-Members
...
## 35. Signal Separation: Unmixing Overlapping Peaks
...
## 36. Transfer Learning for Signals: Synthetic to Real
...
## 37. Robustness to Instrumental Shifts
...
## 38. Summary: Signal Processing Pipeline
...
## 39. Non-Negative Matrix Factorization (NMF)
...
## 40. Case Study: Automatic XRD Phase Identification
...
## 41. Case Study: EELS Spectrum Imaging — Fe$^{2+}$/Fe$^{3+}$ Mapping
...
## 42. Case Study: Large-Scale EDS Maps
...
## 43. Clustering in Latent Space: UMAP Visualization
...
::: {.fragment}
5. **Masked Autoencoders** [@he_2022_mae] turn the lab's $10^4$–$10^5$ **unlabelled** spectra into a pretrained feature extractor — small-data property prediction with linear probing or LoRA
:::

::: {.fragment}
6. The **latent space** is the universal interface: it enables compression, denoising, clustering, anomaly detection, and visualization
:::

::: {.fragment}
7. **Practical pipelines** require careful normalization, validation, and robustness to instrumental shifts
:::
...
- &rarr; Next: [Unit 09b &mdash; Transformers for materials (ViT, Flash Attention, Mamba)](../unit09b_transformers_for_materials/transformers_for_materials.html)
