...
**The week-numbering note (say it once, do not belabor).** The folder on disk is still called `unit08_…` because we deferred renames to term-end after the SS26 realignment. The slide deck and the calendar both call this Unit / Week 7. If a student notices and asks during office hours, the explanation is: this material is the natural pair of MFML W7 (Generalization, Regularization, Model Selection) and we moved it earlier so the two tracks reinforce each other.
...
- Unit 12 → Gaussian Processes give us calibrated uncertainty, which is the principled version of the "probability contour" we will draw freehand today.
- Unit 13 → physics-informed losses are a regularizer; we will preview the Bayesian view here.
...
**What I am not covering today.** Calibrated uncertainty (Unit 12), conformal prediction (Unit 12), causal sensitivity (Unit 14). I will name them when they come up so students can tell which concept lives where.
:::
...
**Cross-reference.** This is the same picture students saw in MFML W6 (training NNs) and they will see again in Unit 12 (GPs, where the validation curve is replaced by a marginal likelihood).
:::
...
**Pelz-group case in point.** 4D-STEM datasets routinely exceed $10^{10}$ entries; ptychographic reconstruction networks operate in this regime. The only reason they generalize is that they bake the *forward model* in (Unit 9 / Unit 13 preview).
...
- *Conservation* — mass, charge, momentum (Unit 13 will formalize this).

**Forward link.** Regularization comes back as the *physics-informed loss term* in PINNs (Unit 13) and as the *kernel choice* in GP regression (Unit 12).
:::
...
**Aleatory vs epistemic — the punchline.** Aleatory you cannot fix with more data of the same kind; epistemic you *can*. A robust model should *report which kind* of uncertainty dominates at a query point. We come back to this in §4 (process windows) and Unit 12 (GPs).
...
**Anti-pattern.** Reporting a single softmax probability as "the model's confidence." Modern NNs are notoriously *miscalibrated*: a softmax of 0.95 does not mean 95% chance correct. Calibration (Platt scaling, temperature scaling, isotonic regression) and conformal prediction are the post-hoc fixes — Unit 12.
:::
...
**Practical advice.** Build distribution-shift monitoring into deployment from day one. Track feature statistics in production; raise an alarm when they drift more than a few standard deviations from training. This is the closed-loop view we will revisit in Unit 11 (automation) and Unit 12 (uncertainty + active learning).
:::
...
**Forward link to MFML.** The Bayesian view (Unit 12) handles this naturally: a heavy-tailed likelihood absorbs occasional large residuals without panicking, and the posterior tells you which observations are "surprising."
:::
...
**Time-aware splits.** Process-monitoring data (Unit 7) has a temporal causality students often violate. If your build runs from 0:00 to 8:00 and you split randomly, the model sees the future — leak. Walk-forward (train on 0:00–1:00, test on 1:00–2:00; train on 0:00–2:00, test on 2:00–3:00; …) is the only honest protocol.
:::
...
**A fourth set you sometimes need: the "production" set.** Real deployment data, collected after the paper is published. The honest test of generalization. We will revisit this in Unit 12 (active learning) and Unit 14 (closing the loop).
:::
...
**Forward link.** The surrogate is a Gaussian Process — that is *literally* Unit 12. Bayesian optimization is one of the most concrete applications of probabilistic ML in materials science (autonomous experiment design, autonomous synthesis).
...
4. Updating the boundary online as new data arrives (active learning, Unit 12).
...
**Active learning preview.** Where is the next experiment most informative? At the boundary, where the model is uncertain. This is the closing of the loop — Unit 12.
:::
...
The 95% contour conflates these — for many production decisions that is fine, but in §5 (sensitivity) and Unit 12 (GPs) we will pull them apart.
:::
...
**Forward link.** Unit 12 derives the exchangeability proof and adds **CQR** (Conformalized Quantile Regression) for adaptive widths in heteroscedastic regimes. Today's slide is the elevator pitch.
...
**Unit 11 link.** The full closed loop — sense, decide, act — is the topic of the automation unit. Today we are showing where the *decision* layer comes from: the ML-derived process window.
:::
...
**Bridge to Unit 12.** The whole answer hinges on calibrated uncertainty. That is the central topic of Unit 12 — Gaussian processes specifically, and Bayesian methods generally. Today's lecture motivates *why* you would invest in the harder methodology.
:::
...
**Why I am only previewing it today.** Full treatment is in Unit 14 (interpretability and reflection). Today's role: SHAP is the *gold standard* for tabular feature attribution; it generalizes permutation importance and gradient × input by averaging over all feature subsets.
...
**Why Unit 8, not Unit 14.** §5 (Sensitivity & Feature Importance) is the natural home — this is the next-generation feature-importance tool. Unit 14 (Reflection) will revisit it from the ethics / certification angle.
...
**Forward link.** Unit 14 mech-interp slide; Unit 5 sparse-autoencoder exercise (planned in the lecture-content modernization).
:::
...
Forward: Unit 8 (inverse problems) uses today's process windows as constraint sets. Unit 12 (GPs) gives us calibrated uncertainty so the windows are honest. Unit 13 (PINNs) tightens the regularizers with physics priors. Unit 14 (reflection) deepens SHAP and discusses ML's epistemic limits.
:::
...
- &rarr; Next: [Unit 09 &mdash; Inverse problems and process maps](../unit09_inverse_problems/09_inverse_problems.html)
...
**Connection to the *next* lecture.** Unit 8 (Week 8) is inverse problems. We will use today's *forward* models (process → property) and today's *process windows* (the constraint sets) and ask the inverse question: *given* a target property, *what* process parameters get us there? Bring today's notes.
:::
