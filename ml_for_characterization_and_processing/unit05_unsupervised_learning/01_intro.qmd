...
::: {.notes}
**Why this case study sits here.** Slide 7 gave you the "low-dim, hand-picked geometric features" worked example; slide 12 (Hall–Petch) will give you a 2-D feature space where K-means finds the right answer almost trivially. ESTM is the bridge: a real, public materials database where the feature space is *not* obvious, and the featurization design choice will dominate the result. Once students see this, the §B claim ("representation beats algorithm" on micrographs) lands without resistance.
...
::: {.columns}
::: {.column width="50%"}
![PCA-2D of element-fraction (left) and Magpie (right) features, points coloured by ZT (viridis, 0 → 99th percentile). High-ZT pockets jump out as bright regions of the Magpie map.](images/estm/pca_scatter_by_zt.png){width=100%}

![Median ZT vs measurement temperature, one line per Magpie cluster (clusters with $<20$ points suppressed). Different families peak in different T windows.](images/estm/zt_by_cluster_vs_T.png){width=100%}
:::
::: {.column width="50%"}
...
**Forward link to §B and Unit 9.** Magpie was designed by humans encoding physical intuition into 132 statistics. Could a neural network do better? Yes — Na & Chang's SIMD descriptor learns the cluster-aware projection from a similarity graph over materials, and pushes extrapolation further than Magpie alone. That is exactly the "learned representations" idea that §B introduces on micrographs (slide 14+) and that Unit 9 makes the centrepiece (latent spaces, contrastive learning). Today we have the hand-designed baseline; the next two units upgrade it.
...
- "I clustered SEM images of a *tensile test series* into 3 phases." There aren't 3 phases — there's a continuous evolution of strain. The clusters are arbitrary slices through a one-parameter continuum. The right tool is regression on strain, or visualisation of an AE bottleneck (slide 43).
...
**Pre-empt.** "But the CNN was trained on cats and dogs!" Yes, and that is exactly the caveat we will flag in slide 25. For now: it works often enough to try first.
:::
...
::: {.columns}
::: {.column width="50%"}
**Internal — no labels needed (used to pick $k$)**
...
:::
::: {.column width="50%"}
**External — require labels (used to score against truth)**
...
**Used on slides 20–22 to score the NEU-DET case study against ground truth.**
:::
:::

::: {.notes}
**Why this slide exists where it does.** Silhouette and BIC have already been name-dropped (slides 7, 12, 17) as the way you pick $k$ when no labels are available. ARI and NMI are about to appear on the NEU-DET case study (slides 20–22) as the way you score a clustering *when labels happen to exist*. One slide, both pairs, before either is needed in anger.

**The conceptual split, said aloud.** "Internal vs external" is the load-bearing distinction. Internal metrics need only the data and the partition — you can compute them on any clustering, always. External metrics need labels — you can compute them only sometimes, typically post-hoc on a small evaluation subset. Workflow: pick $k$ with internal; *if* labels arrive later, validate with external. Never the other way around (that would be a leak).
...
**Reference numbers for context.** On ImageNet-pretrained features clustered with K-means: ARI $\geq 0.7$ is "good", $\geq 0.85$ is "excellent". Pure-pixel features on the same task: ARI typically $< 0.2$. The NEU-DET case study (next two slides) shows exactly this gap.

**`sklearn` one-liners.** `adjusted_rand_score(y_true, y_pred)`; `normalized_mutual_info_score(y_true, y_pred)`; `silhouette_score(X, labels)`; `GaussianMixture(...).bic(X)`. All run in milliseconds; report all four when you have labels.

**One trap to mention.** Silhouette uses Euclidean distance by default. If you've L2-normalised embeddings (slide 16), Euclidean ≡ cosine up to monotone — fine. If you haven't, silhouette on raw 512-D vectors can be misleading; standardise first.
:::

## 19. Worked Example: Phase Discovery in SEM
...
## 20. Case Study — NEU-DET Steel Defects (1/3)
...
**Connect to slide 17.** Pipeline on this slide *is* the §B pipeline from slide 17: embed → standardise → cluster → evaluate. The only thing new here is a real dataset and side-by-side comparison with raw features. The next slide shows what comes out.
:::

## 21. Case Study — NEU-DET Feature Pipelines (2/3)
...
## 22. Case Study — NEU-DET Results (3/3)
...
::: {.fragment}
> The encoder did the work. We will revisit this on slide 25 (caveat) and slide 26 (domain-pretrained encoders).
:::
:::
:::
...
**The scratches/inclusion confusion is real, not noise.** Look at the example tiles on slide 20: both classes are elongated dark streaks on light backgrounds. ImageNet has no concept of "this dark streak is a foreign-material inclusion vs a mechanical scratch" — it sees both as "elongated dark thing". A domain-pretrained encoder (slide 26, Unit 9) closes this gap.
...
**Bridge to slide 23.** "Now imagine you didn't have ground-truth labels — which you usually don't. The next slide shows how clusters that contain very few samples are themselves diagnostic: they surface the dataset's *outliers*."
...
## 23. Outlier Detection via Singleton Clusters
...
## 24. Quality Gate for Automated Microscopy
...
## 25. Caveat: Cluster Meaning ≤ Encoder Quality
...
**Bridge to slide 26.** The last fix on the list is itself an *unsupervised method*: contrastive learning. That is the §B-content of Unit 9, four weeks from now.
:::

## 26. Forward Pointer: Domain Encoders & Contrastive Learning
...
## 27. Hyperspectral Data in Materials
...
**Scale of data students will hit.** A 4D-STEM EELS dataset is routinely 50 GB. Loading it as a dense $H \times W \times C$ array in Python is feasible; storing it that way long-term is not. Compression (slide 40) becomes operational, not optional.
:::

## 28. Reframe as Clustering
...
**Pitfall to flag (revisited in slide 32).** Because spatial coherence isn't enforced, clusters can speckle — single pixels deep inside a phase region get wrong assignments due to noise. Remedies (median filter on cluster map, MRF-like priors, deep-learning approaches to coupled spatial-spectral clustering) are flagged for the exercise.
...
## 29. K-means Phase Maps — Duplex Stainless Steel by Low-Loss EELS

::: {.columns}
::: {.column width="45%"}
**Setup [@castro_riglos_2024]**

- Industrial **duplex stainless steel** thin foil in STEM.
- Low-loss EELS spectrum image: $\sim 10^4$–$10^5$ pixels, plasmon-region channels ($\sim 0$–$50$ eV).
- Pre-process: zero-loss alignment, deconvolution, per-spectrum sum-normalise.
- **K-means**, $k$ chosen by silhouette over $k \in \{2, \dots, 6\}$ → $k = 3$.

**What each cluster *is***

- Centroid $\mu_k$ is a *prototypical low-loss EELS spectrum* — physically interpretable.
- Re-imaged cluster IDs → phase map at the **acquisition pixel** scale.
:::
::: {.column width="55%"}
![Low-loss EELS K-means phase map of industrial duplex stainless steel. The three clusters correspond to the ferrite (α) matrix, austenite (γ) islands, and σ-phase precipitates. Adapted from Castro Riglos et al., *Materials Characterization* **211**, 113924 (2024).](images/eels_kmeans_duplex_steel.png){width=100%}
:::
:::

**Outcome.** Recovered phases: **ferrite (α)**, **austenite (γ)**, and **σ-phase precipitates**. Phase map agrees with co-acquired EDS, HAADF, and electron diffraction. **Runtime: K-means ≈ 30 s vs ≈ 10 min for pixel-wise spectral fitting** on the same dataset.

::: {.notes}
**Why this paper is the right worked example.** Three reasons. (1) It is a *real* industrial dataset — duplex stainless steels are a major structural-materials class, σ-phase precipitates control toughness, and operators *care* about the phase map. (2) The pipeline is exactly what we built on slide 28: flatten cube → K-means → reshape → done. No tricks. (3) The benchmark vs pixel-wise spectral fitting is a $\sim 20\times$ speedup — the kind of number that lands with an audience of engineers.
...
**How they chose $k = 3$.** Silhouette sweep over $k = 2$ to 6; clear maximum at $k = 3$. Same diagnostic logic as slide 18 (cluster quality metrics). They cross-checked against EDS-derived phase fractions and electron-diffraction-confirmed crystal structures — agreement is the criterion that promotes "clusters" to "phases".
...
## 30. GMM Phase Maps and Mixed Pixels
...
**Bridge to slide 31.** The compositional interpretation of $\gamma_{i\cdot}$ leads directly to *spectral unmixing*: model each pixel as a non-negative sum of physically-meaningful endmember spectra. GMM with a Gaussian likelihood is too unconstrained for that; we need positivity + sum-to-one. That is the topic of the next slide and a forward pointer to W9.
...
## 31. Spectral Unmixing as Constrained Clustering
...
:::
::: {.column width="50%"}
**Worked example — STEM-EDXS of a deep-mantle assemblage [@chen_2024_nmf]**

- Sample: synthetic mineral assemblage matching Earth's lower mantle — *bridgmanite*, *ferropericlase*, *Ca-perovskite*.
- Phases overlap *spatially* (sub-pixel intergrowth) and *spectrally* (shared X-ray lines for Fe, Mg, Ca, Si, O).
- NMF with physics-guided initialisation → recovers endmember spectra **and** abundance maps **and** trace-element quantification down to **$\sim 100$ ppm**.
- Plain K-means/GMM would mis-assign mixed pixels to whichever phase dominates; NMF gives the actual fractional composition.

![Conceptual: NMF decomposes a STEM-EDXS datacube ($\mathbf{S}$, $N \times C$) into $K$ non-negative endmember spectra ($\mathbf{M}$, $K \times C$) and a non-negative abundance map per phase ($\mathbf{A}$, $N \times K$). Each pixel $i$ is reconstructed as $\mathbf{s}_i \approx \mathbf{A}_{i,:} \mathbf{M}$. Placeholder — drop in Fig. 5 of Chen et al. (2024) for the deep-mantle abundance maps.](images/nmf_unmixing_concept.png){width=100%}
:::
:::
...
## 32. Practical Pitfalls in Hyperspectral Clustering
...
## 33. From Clustering to Representation Learning
...
**The single most important sentence of the unit.** *The autoencoder bottleneck is the conceptual through-line of the next two months.* Repeat this aloud at slide 33 and again at slide 53.

**Pacing.** §D is the longest section (11 slides, ~20 min). Slow down here. The math (slide 34) and the architecture (slide 35) are the load-bearing slides; slides 36–43 are applications and diagnostics, which can move faster.
:::

## 34. Autoencoder Objective
...
**Foreshadow.** "$z$ is the most important thing we'll define today. We use it for denoising (slide 38), compression (slide 40), feature extraction (slide 41), anomaly detection (§E), and — in the W11 generative deck — sampling. Hold $z$ in your head."
:::

## 35. Convolutional AE Architecture
...
## 36. Bottleneck Dimension as Inductive Bias
...
## 38. Application 1 — Denoising Low-Dose TEM
...
## 39. Denoising Results: Reporting
...
**Failure modes.** (1) Structured noise, see slide 38. (2) Distribution shift — train on one microscope, deploy on another with different PSF / detector statistics → denoiser hallucinates training-set textures. Always validate on a held-out *session*, not just held-out frames.
:::

## 40. Application 2 — Compression for Archival
...
## 41. Application 3 — AE Features for Downstream Regression
...
## 42. Diagnostic: Reconstructions of Held-Out Frames
...
## 43. Bottleneck Visualisation Preview
...
**The anti-cluster.** If the 2D scatter shows a *continuum* — long curve, no discrete groups — that is *not* a failure. It is the dataset *telling you* it has no discrete clusters, just a continuous variation. Use the AE coordinate to predict properties (slide 41); do not force K-means on it.
...
## 44. The Anomaly-Detection Setup
...
::: {.notes}
**The conceptual leap of §E.** Until now we used the AE to *represent* data ($z$, slide 43) or to *reconstruct* clean data (slide 38). Section E uses the *failure mode* of the AE — its inability to reconstruct out-of-distribution input — as the actual signal. The bug is the feature.
...
## 45. Anomaly Score: Frame-Level vs Pixel-Level
...
## 46. Threshold Selection from Nominal Validation
...
## 47. Case Study — Melt-Pool Monitoring (LPBF, 1/2)
...
## 48. Case Study — Melt-Pool Monitoring (LPBF, 2/2)
...
## 49. Case Study — SEM Long-Run Anomaly Flagging
...
**Distinct from §B's K-means quality gate (slide 24).** Both serve as quality control. K-means clustering catches anomalies that fall *outside the embedded nominal cluster* — anomalies *of kind*. AE catches anomalies that fall *outside the AE's learned manifold* — anomalies *of appearance*. Use both in parallel; they catch different things.
...
## 50. Failure Mode 1 — Training Set Too Narrow
...
## 51. Failure Mode 2 — Anomaly Leakage into Training
...
**A common version of this in LPBF.** "Nominal" builds verified by CT may have sub-CT-resolution pores that the AE learns to compress. Sub-CT pores then go undetected at deployment. Mitigation: pixel-level rather than frame-level scoring (slide 45), since localised sub-CT pores still produce localised AE residuals.

**Recap of slides 50–51, said aloud at the end.** "False positives = training set too narrow. False negatives = anomalies in training set. Both kill deployments. Both are *data* problems, not *algorithm* problems."
:::

# §F · Wrap-Up {.section}

## 52. Method-vs-Application Matrix
...
- **Anomaly detection:** AE wins decisively (§E). Clustering can flag outliers via singletons (slide 23) but is far less sensitive.
- **Compression:** AE only. Clustering is discrete by definition; no information for reconstruction.
- **Downstream regression:** AE features (slide 41) win when bulk unlabeled data is available; CNN embeddings (slide 17) win when ImageNet transfer is good enough; classical features (§A) win when the descriptor is itself physically meaningful.
...
## 53. Forward Pointers — The Bottleneck Through-Line
...
::: {.notes}
**Repeat the through-line.** I said it at slide 02 and slide 33; I say it again at slide 53 because *this is the lecture-level summary*. The bottleneck is the conceptual object that ties the second half of the course together.
...
## 54. Exercise + Reading Assignment
