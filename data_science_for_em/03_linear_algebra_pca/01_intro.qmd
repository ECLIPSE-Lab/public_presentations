...
::: {.columns}
::: {.column width="55%"}
![A $2\times2$ matrix applied to the unit square: rotates, stretches, and shears.](img/vectors_projection.png){width="100%"}
:::
::: {.column width="45%"}
:::: {.incremental}
...
# X: data matrix, shape (N, D) — observations in rows, features in columns
# (already centered: X_centered = X - X.mean(axis=0))
...
# Reconstruct (denoise): keep only the signal subspace
X_denoised = scores @ eigenspectra + X_centered.mean(axis=0)  # WRONG without centering fix
```

. . .

```python
# Correct full pipeline including mean restoration
mean_spectrum = X.mean(axis=0)             # (D,) — mean of original (un-centered) data
X_denoised    = (U[:, :K] * s[:K]) @ Vt[:K] + mean_spectrum   # shape (N, D)
```
...
:::: {.notes}
- Walk through the figure in detail. The two left bars (k=1 and k=2) are clearly taller than the rest — these capture the two latent EELS components. From k=3 onwards the bars are nearly equal height (the noise floor). The elbow is between k=2 and k=3.
...
::: {.column width="45%"}
:::: {.incremental}
- **Setup:** 48-pixel × 200-channel synthetic EELS line scan.
- **Two latent components:** Fe-L edge (~710 eV) dominant in the centre; Cr-L edge (~580 eV) dominant at the edges.
- **Noise:** Poisson (scale = 300 counts); realistic low-dose EM conditions.
