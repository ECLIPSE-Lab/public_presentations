...
:::: {.incremental}
- **Road map:** recap Week 5 + today's question (2) · why MLPs fail on images (4) · convolution as a sliding feature detector (5) · kernels as edge/texture detectors (4) · stride, padding, channels (4) · weight sharing and translation equivariance (4) · pooling and downsampling (3) · receptive field and feature hierarchy (4) · CNN architectures: LeNet → AlexNet → ResNet (4) · U-Net for segmentation (5) · EM case study: phase and grain segmentation (5) · practicalities and failure modes + Week 7 preview (3).
...
- Misconception to preempt: "larger kernels always see more." Two 3×3 layers with a ReLU between them have a larger effective receptive field AND more nonlinear expressivity than one 5×5 layer.
...
![U-Net applied to TEM images of Au nanoparticles in an amorphous silica matrix. Left: input TEM image (synthetic, representative of SHINE dataset). Centre: ground-truth binary mask (crystalline=bright, amorphous=dark). Right: U-Net prediction — pixel-wise classification matching the ground truth closely.](img/unet_em_casestudy.png){width="85%"}
...
- **How it works:** freeze the first three blocks of a pretrained CNN (learned edge, texture, and motif detectors); fine-tune the final classification head on the target EM dataset.
- **Why it works:** edges, textures, and local motifs in natural images share statistical properties with edges, textures, and local motifs in EM images — even though the physical origin is completely different.
- **Output:** a feature vector (embedding) per image, enabling clustering, classification, and similarity search.
::::

:::: {.notes}
- Explain transfer learning at a high level: "a CNN trained on ImageNet (1.4 million natural photos) has learned edge detectors, texture filters, and local motif detectors in its first layers. These are also useful for EM images. We keep those layers and replace only the final classification head with a small network for our specific task."
- The one point to land: this is the Week 7 preview — you do not need to understand the fine-tuning mechanics today, only that the feature hierarchy (edges → motifs → structures) transfers across domains.
- EM anchor: several published workflows for steel microstructure classification (martensite vs bainite vs ferrite) use ResNet-18 pre-trained on ImageNet and fine-tuned on ~200 labelled micrographs [@decost2017microstructure].
...
- A U-Net for 256×256 binary segmentation: ~7.8 million parameters total.
