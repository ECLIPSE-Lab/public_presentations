...
- Pacing note: batch the learning-taxonomy slides quickly and protect at least 15 min at the end for the noise/sampling section — that is where the core physical intuition lives.
...
:::: {.notes}
- The standard pipeline for 4D-STEM phase segmentation is PCA compression followed by K-means clustering — we build this in Week 3.
- The one point to land: unsupervised learning is not a theoretical curiosity — it is a daily tool in EM labs for phase mapping without manual annotation.
- Misconception to preempt: "K-means tells you how many phases are present." K-means requires you to specify K in advance; that is a materials science decision, not a mathematical one.
- EM anchor: clustering EELS spectra or 4D-STEM diffraction patterns automatically segments images into chemically or crystallographically distinct regions.
- Forward link: Week 3 covers PCA in depth. Week 8 covers autoencoders as a non-linear alternative.
...
::: {.columns}
::: {.column width="50%"}
![Simulated STEM phantom at four dose levels (left to right: 5 000, 500, 50, 5 e⁻/Å²). Image-average SNR ≈ 29/9/3/1; the lowest-dose image is dominated by Poisson noise.](img/dose_snr_simulation.png){width="100%"}
:::
::: {.column width="50%"}
:::: {.incremental}
...
- At 5 000 e⁻/Å²: columns clearly resolved; SNR ≈ 29.
- At 500 e⁻/Å²: columns visible but noisy; SNR ≈ 9.
- At 50 e⁻/Å²: columns barely visible; SNR ≈ 3.
- At 5 e⁻/Å²: columns invisible to the eye; SNR ≈ 1. ML denoising required.
