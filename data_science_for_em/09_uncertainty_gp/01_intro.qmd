...
![Left: aleatoric uncertainty — grain-to-grain scatter in grain size measurements at a fixed annealing temperature. More measurements reveal the *distribution* of grain sizes but do not eliminate the scatter (it is real). Right: epistemic uncertainty — a GP fitted to only 3 EELS measurements (diamonds) is uncertain everywhere except near the measured compositions; measuring more narrows the band. [@bishop2006pattern]](img/aleatoric_epistemic_em.png){width="82%"}
...
![Left: GP fitted to 8 EM measurements clustered in [0.05, 0.85]. The 95% credible band widens rapidly beyond the data range (red shading) — the GP honestly admits it has no information there. The highest-uncertainty regions are the best candidates for the next measurement. Right: after adding one measurement at x=1.0, the local band collapses (50% reduction in σ* there) while regions further away remain uncertain.](img/gp_uncertainty_balloons.png){width="88%"}
...
- The 50% reduction number: from the notebook, σ* at x=1.15 goes from 0.1966 to 0.1003 after adding the point at x=1.0. This is a real, notebook-verified number.
...
- **Small calibration sets give uncertain guarantees.** Split conformal with $n_\text{cal} = 50$ gives a coverage guarantee of ≥ 90% with uncertainty. With $n_\text{cal} = 200$ the coverage is tighter (closer to exactly 90%). Use the largest feasible calibration set.
...
- Small n_cal: the conformal coverage guarantee is ≥ (1-α)(1 + 1/n_cal). With n_cal=50 and α=0.10, the guarantee is ≥ 0.90 × (1 + 1/50) ≈ 0.918 — the target may be missed by up to 1.8 percentage points due to the finite-sample correction.
::::
