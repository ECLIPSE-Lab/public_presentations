...
- **Self-study:** `notebooks/week05_tiny_mlp.ipynb` — train a small MLP on a materials-property dataset and watch it beat a linear baseline.
::::
...
- Misconception to preempt: "a deep network always beats hand-crafted features." It does not — especially when $N$ is small. The Hall–Petch linear baseline achieves $R^2 \approx 0.90$ on this dataset with only 140 training samples. An MLP needs more data to outcompete it.
...
- Forward link: next slide shows the comparison visually. The lesson is not "MLP wins" but "both approaches are valid; the right choice depends on how much domain knowledge you have."
- Transition: "Let me show the comparison visually."
::::

## Hall–Petch: hand-crafted vs learned features

![Left: linear regression on the hand-crafted $1/\sqrt{d}$ feature achieves $R^2\approx0.90$. Right: an MLP trained on raw grain size $d$ recovers the same nonlinear trend from data alone — approaching comparable performance with 200 training samples.](img/hall_petch_motivation.png){width="80%"}

:::: {.notes}
- Walk through the two panels. Left: "We told the model the functional form. It does the least-squares fit. Result: $R^2 \approx 0.90$ with 140 training points." Right: "We gave the model raw $d$ and let it find the transformation. It has learned the $d^{-1/2}$ shape without being told."
