...
<!-- ===== §1. Computational imaging = optics + sensor + computation (3 slides) ===== -->
...
![Comparison of L2 (Tikhonov) and total-variation (TV) regularisation on a 1-D signal representing a grain boundary step (piecewise constant truth: value 0.2, then 0.8, then 0.3). Tikhonov (blue, left) recovers the signal but smooths the sharp transitions into broad ramps. TV (orange, centre) preserves the sharp step edges at the cost of introducing slight staircase texture in the flat regions. Both are compared to the true signal (black, right). Neither is perfect — the choice of prior determines which artefact appears.](img/tikhonov_vs_tv.png){width="82%"}
...
![L-curve for the 1-D deblurring problem: horizontal axis = residual norm $\|Hx̂ - y\|$ (data fit), vertical axis = solution norm $\|x̂\|$ (solution complexity), both in log scale. Each point on the curve corresponds to a different $\lambda$ (colour-mapped from blue = large $\lambda$ to yellow = small $\lambda$). The curve has a characteristic L-shape. The corner (red dot, $\lambda \approx 24.7$) is the point of maximum curvature in log–log space — the optimal balance between data fit and solution norm. Blue triangle (bottom-right): small $\lambda$ = under-regularised, very tight data fit but enormous solution norm. Green triangle (top-left): large $\lambda$ = over-regularised, very small solution norm but poor data fit.](img/lcurve.png){width="65%"}
...
- Note the difference between the L-curve optimal λ≈24.7 and the RMSE-optimal λ≈0.013 in the notebook. They are different because the L-curve uses the solution norm ||x||, not the reconstruction error. The L-curve is a data-only heuristic — it does not require ground truth but is not always perfectly aligned with the RMSE minimum. This honest discrepancy should be mentioned: the L-curve is a guide, not a guarantee.
