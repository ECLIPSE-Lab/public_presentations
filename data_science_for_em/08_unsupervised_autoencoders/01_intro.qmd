...
![Denoising performance on synthetic low-dose EELS spectra near the Fe-L₂₃ edge (700–730 eV). Top-left: clean ground-truth spectrum (two Gaussian peaks). Top-right: noisy input at 50 electrons/channel (Poisson noise; SNR ≈ 7). Bottom-left: PCA denoising (rank-1 projection) — recovers the main peak but misses the weaker shoulder. Bottom-right: denoising AE — recovers both peaks with significantly lower reconstruction error. Green dashed line is the clean reference.](img/denoising_ae_spectra.png){width="82%"}

:::: {.notes}
- Walk through the four panels. Emphasise the bottom-left vs bottom-right comparison: PCA misses the weaker shoulder because it cannot capture the relative peak-area ratio non-linearly; the AE recovers both.
...
- The figure caption claims both peaks are recovered by the AE. This is exactly what the figure shows — the green dashed line is aligned with both the AE reconstruction and the clean truth.
...
![AE latent space (z₁ vs z₂) for 240 synthetic iron-oxide EELS spectra (3 phases, 80 spectra each). The three phases (Fe₂O₃ red, FeO blue, Fe₃O₄ orange) cluster into well-separated islands without any labels — the only information the AE received was the raw spectra and the instruction to reconstruct them. Stars mark the cluster centroids found by k-means applied to the latent codes.](img/latent_space_phase_clustering.png){width="62%"}
...
![Left: reconstruction error distribution for 500 normal iron-oxide spectra (blue) and 30 anomalous spectra from beam-damaged regions (red). The AE was trained on normal spectra only. Anomalies reconstruct poorly — they lie outside the manifold the AE learned. The 99th-percentile threshold (orange dashed line) flags anomalous spectra without any labelled defects in the training set. Right: reconstruction-error spatial map with high-error sites (red contour) corresponding to beam-damage hotspots.](img/anomaly_detection.png){width="85%"}
...
- **Exercise:** sweep bottleneck dimension 1–8; observe AE reconstruction error decrease and silhouette score peaking near dim=4 (the true number of phases).
- **Assert checks:** AE recon error < noisy input; AE < PCA recon error; AE silhouette > 0.5; AE silhouette > PCA silhouette — all pass on SEED=42, 300 epochs.
::::
