...
:::: {.notes}
- Read the roadmap at medium pace. Two anchors to call out: (1) "acquisition function = explore vs exploit score" — that phrase is the conceptual core of the whole week; (2) "BO escapes the deceptive local optimum and finds the global peak in 12 iterations" — the honest notebook result. Everything else supports those two payoffs.
- One pacing note: this slide is orientation — 1 minute maximum. Do not read every bullet. Gesture at the structure.
- The notebook numbers (0.9323 vs 0.7229) are real and match the executed notebook (SEED=42, N_INIT=3, N_ITER=12, κ=3.0). Mention them now to give students a concrete target.
...
- **Limitations:** $O(N^3)$ cost (fine for $N \leq 500$ EM measurements); sensitive to kernel choice; struggles in $> 10$ input dimensions. For image-patch inputs, use DKL (later today). [@rasmussen2006gaussian]
::::
...
- **The inner loop:** after each new observation $(x_\text{next}, y_\text{next})$, the GP is updated: $\mathbf{K}$ gains a new row and column; the matrix inverse is updated (Cholesky update in practice — $O(N^2)$, not $O(N^3)$). [@rasmussen2006gaussian]
::::
...
- **κ = 5.0 (high exploration):** also escapes; wider initial search. Notebook result with SEED=42: best found **0.9331**. [@shahriari2016taking]
...
:::: {.incremental}
- **The key insight from Roccapriore et al. [@roccapriore2022physics]:** the scalariser function encodes the *physics* the experimenter cares about. "Find the region of maximum local electric field" is a physics goal encoded as `scalariser = CoM_magnitude`. The DKL + BO system then pursues that goal autonomously.
...
- **Not examined for this course:** the mathematical derivation (policy gradient theorem). The conceptual points — (1) RL learns from rewards, no labels; (2) gradient ascent on expected return; (3) the same backprop the students know from Week 5 — are all that is required. [@bishop2006pattern]
::::
...
- **RL advantage over manual:** manual alignment requires an experienced operator to interpret diffraction patterns and know which control to adjust. The RL policy learns the causal chain automatically from the reward signal — it can run while the operator is absent. [@roccapriore2022physics]
::::
...
:::: {.incremental}
- **The promise:** a self-driving TEM runs 24/7. The operator defines an objective ("find all Ni₃Al precipitates and measure their size distribution"). The instrument acquires, analyses, and reports without human intervention. [@roccapriore2022physics]
...
- &rarr; Next: [Week 11 &mdash; Imaging inverse problems I](../11_inverse_problems_1/01_intro.html)
- &larr; Back: [Week 09 &mdash; Probability, uncertainty &amp; Gaussian processes](../09_uncertainty_gp/01_intro.html)
