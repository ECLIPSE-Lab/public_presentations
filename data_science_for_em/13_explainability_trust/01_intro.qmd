...
## Saliency maps: clean model vs shortcut model

![Left: EM image with lime-circle defect region. Centre: gradient saliency map. Right: overlay. Top row (clean model): saliency concentrated inside the defect (ratio 3.2×). Bottom row (shortcut model): saliency concentrated at the corner artifact (ratio 9.3×) — the model learned a spurious shortcut.](img/saliency_clean.png){width="80%"}

:::: {.notes}
- This is the centrepiece figure of the lecture. Walk through it carefully.
- Top row: the clean model was trained on images where the defect IS the only distinguishing feature. Its saliency map peaks inside the lime-dashed circle (inside = 0.627, outside = 0.195, ratio 3.2×). The model is doing the right thing physically.
- Bottom row: the shortcut model was trained on images where the corner artifact (NOT the defect) was the distinguishing feature. Its saliency map peaks at the top-left corner (corner = 0.500, defect region = 0.054, ratio 9.3×). The model has learned a spurious pattern — it will fail if deployed on images without the corner artifact.
- The diagnostic power: both models achieve ~99% test accuracy on their own test distribution. Without saliency maps, you cannot tell them apart. This is the entire motivation for XAI in EM.
- Calibration tie-in: the shortcut model is also likely to be poorly calibrated on out-of-distribution images (microscopes without the corner artifact). Confidence ≠ correctness once you leave the training distribution.
::::
...
![Saliency at the corner artifact (red box) instead of the defect (lime circle) — the shortcut model's explanation looks spatially coherent but physically wrong. Accuracy 0.990; saliency corner/defect ratio 9.3.](img/saliency_shortcut.png){width="80%"}
...
- **Integrated Gradients (IG)** [@sundararajan2017ig] — a pixel-level attribution for CNNs that satisfies the completeness axiom:
  ...
  where $x_0$ is a baseline (e.g. black image), and $x$ is the input to explain.
- **Completeness:** $\sum_i \mathrm{IG}_i(x) = f(x) - f(x_0)$ — the pixel attributions sum exactly to the model output minus the baseline. Same as SHAP completeness, but for pixels.
...
:::: {.notes}
- For this cohort: the formula is REFERENCE ONLY — not examined as a derivation. The exam deliverable is: "IG integrates the gradient along a path from baseline to input; satisfies completeness; more stable than raw gradient saliency." That is the exam-level answer.
...
- Do not derive the integral — just say "instead of using the gradient at one point, integrate the gradient along the path from a black image to the actual input; average over that path." The key intuition is that raw gradients can be noisy at any single point; integration smooths over the path.
